# Cliente de escritorio Mirror Shift 1.0

[English](README.md) · [Documentación del proyecto](../docs/README.es.md)

Cliente Qt de Mirror Shift 1.0 construido sobre el runtime de Shatranj.
Windows, macOS y Linux utilizan la misma implementación y admiten ambos
transportes:
...
Los botones TAKEBACK y RESET junto a SEND reutilizan las confirmaciones de sesión.
El contador de Echoes aparece dentro del campo de entrada.
...
3. Pulsa una sola casilla resaltada del Lattice cuando sea tu turno. Ese clic
   es un Alignment; Mirrorlock y Silence forzado se resuelven automáticamente.
...
5. Usa estos comandos para acciones de control:
   ...
   /takeback   solicitar deshacer la última jugada aplicada
   /save [name] guardar la posición localmente
   /load [name] solicitar restaurar una posición guardada
   ```

6. Usa los botones de disco bajo el registro de movimientos para guardar o
   cargar una de diez ranuras. Cualquiera puede guardar cuando la sesión está
   lista e iniciar una carga; el otro jugador debe aprobarla y la asignación
   de lado del anfitrión debe coincidir con la sesión actual. Los archivos
   de ajedrez versión 1 se rechazan sin cambiar la partida actual.
...
el peer Direct o MQTT está listo, el estado inferior muestra su plataforma
anunciada como `VS ZX`, `VS NXT`, `VS SPCX`, `VS MAC`, `VS LNX` o `VS PC`; los peers
antiguos aparecen como `VS ?`. Para probarlo contra un anfitrión Spectrum, consulta
...
La capa común posee las reglas de Mirror Shift, el protocolo heredado, la
gramática MQTT, los reducers de sesión y el formato wire v2 de guardado.
...
Instala CMake, Qt con Core/Widgets/Network y un compilador C++ compatible.
En Windows, usa Visual Studio 2022 con soporte C++ y la distribución MSVC de
Qt; apunta `QT_ROOT_DIR` a esa instalación. Los presets MSVC requieren CMake
3.22 o posterior. macOS y Linux usan el compilador y Qt de su plataforma.
...
make tap           # ZX Classic: MIRSHIFT.tap, .OVL y .DAT
make nex           # Spectrum Next: MIRSHIFT.nex autocontenido
...
compila el ejecutable Linux contra el Qt del sistema. El
workflow de GitHub Actions **Build Linux AppImage** empaqueta e inspecciona un
AppImage x86_64 autocontenido y lo sube como artefacto del workflow. Para
publicarlo, descarga ese artefacto y adjunta manualmente el AppImage a la
release de GitHub. En macOS también instala el bundle actual en
`/Applications/MirrorShift.app`; define
...
2. En un host Classic copia juntos `MIRSHIFT.tap`, `MIRSHIFT.OVL` y
   `MIRSHIFT.DAT`; en Next copia el `MIRSHIFT.nex` autocontenido.
...
En Spectrum, la línea de entrada acepta `/resign` y `/takeback`; FILE
