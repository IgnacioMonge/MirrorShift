...
# Each step's output is written to the generated build tree and the log tail is
# printed on failure.
[CmdletBinding()]
param(
...
if ([string]::IsNullOrWhiteSpace($BuildDir)) {
    # Keep the generated tree outside the source tree and out of %TEMP%.
    # On Windows, MSBuild's CompilerId probe can stall in the temporary tree;
    # the persistent LocalAppData build root is also used by client-test.
    $localAppData = [System.Environment]::GetFolderPath("LocalApplicationData")
    if ([string]::IsNullOrWhiteSpace($localAppData)) {
        throw "LocalApplicationData directory is unavailable"
    }
    $BuildDir = Join-Path $localAppData "mirrorshift\pc-dist-build"
}
$DistDir = Join-Path $ProjectRoot "release\MirrorShift"
$PackagedExe = Join-Path $DistDir "MirrorShift.exe"
...
function Remove-GeneratedTree {
    ...
    throw "Unable to clean generated output after ${TimeoutSeconds}s: $resolvedPath. Close running Mirror Shift or locked Qt DLLs and retry. Last error: $($lastError.Exception.Message)"
}

function Assert-PackagedClientClosed {
    if (-not (Test-Path -LiteralPath $PackagedExe)) { return }

    $target = (Resolve-Path -LiteralPath $PackagedExe).Path
    foreach ($process in @(Get-Process -Name "MirrorShift" -ErrorAction SilentlyContinue)) {
        try {
        ...
        if ($runningPath -and
            $runningPath.Equals($target, [System.StringComparison]::OrdinalIgnoreCase)) {
            throw "Mirror Shift is running from $target (PID $($process.Id)). Close it before packaging; no release files were changed."
        }
    }
}
...
function Invoke-ProcessChecked {
    ...
    # Tracker.exe can stall CompilerId/configure on this workstation. Match the
    # canonical dev loop and disable MSBuild file-access tracking for children.
    ...
    try {
        ...
        if (-not $exited) {
            ...
            throw "$Name timed out after ${TimeoutSeconds}s. Close running Mirror Shift or locked Qt DLLs and retry."
        }
        $output = ""
...
function Remove-UnlistedDeploymentFiles {
    ...
    $keepRootFiles = @(
        "MirrorShift.exe", "Qt6Core.dll", "Qt6Gui.dll", "Qt6Network.dll",
        "Qt6Widgets.dll", "concrt140.dll",
...
$buildArguments = @("--build", $BuildDir, "--config", $Config, "--parallel")
if ($env:CMAKE_BUILD_PARALLEL_LEVEL) {
    $buildArguments += $env:CMAKE_BUILD_PARALLEL_LEVEL
}
Invoke-ProcessChecked -FilePath $cmake -Arguments $buildArguments -TimeoutSeconds 300 -Name "cmake build" -LogPath (Join-Path $BuildDir "build.log")

$exe = @(
    (Join-Path $BuildDir "$Config\MirrorShift.exe"),
    (Join-Path $BuildDir "MirrorShift.exe")
...
Copy-SelectedChildren `
    -Source (Join-Path $ProjectRoot "assets\pc-client") `
    -Destination (Join-Path $DistDir "assets\pc-client") `
    -Names @("mirrorshift-wordmark.png", "mirrorshift-icon.png", "about")
Copy-Contents `
