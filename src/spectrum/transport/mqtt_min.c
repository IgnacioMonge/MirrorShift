...
uint8_t netchesszx_asm_mqtt_copy(uint8_t *dst, const uint8_t *src, uint8_t len);
...
static uint8_t mqtt_copy(uint8_t *dst, const uint8_t *src, uint8_t len)
{
    while (len != 0u) {
        if (*src == 0u) { return len; }
        *dst++ = *src++;
        --len;
    }
    return 0u;
                                    ...
                                    uint8_t *flags)
{
    ...
    *flags = (uint8_t)(packet[0] & SPECTRUM_LINK_PAYLOAD_RETAINED);
    ...
    if ((b & 0x80u) != 0u) {
        ...
        if (remaining > (uint8_t)(SPECTRUM_MQTT_PACKET_MAX - 3u)) {
            return -1;
        }
        pos = 3u;
    }
    ...
    if (mqtt_copy((uint8_t *)payload, packet + pos, payload_len) != 0u) {
        return -1;
    }
