...
#define ESCAPE_GUARD_FRAMES \
    ((uint8_t)(net_uart_direct_idle_ticks == 90u ? 66u : 55u))
#else
...
{
    for (;;) {
        uint8_t budget = UART_DRAIN_BUDGET;
        ...
        while (budget-- != 0u && spectrum_uart_ready()) {
            ...
            if (c == '\n') {
                ...
                if (line_pos != 0u) {
                    ...
                    line_pos = 0u;
#ifdef NETCHESSZX_NEXT
                    return (uint8_t)(spectrum_uart_error() == 0u);
#else
                    return 1u;
#endif
                }
...
uint8_t netchesszx_esp_at_test_escape_guard_frames(void)
{
    return ESCAPE_GUARD_FRAMES;
}
...
{
    uint8_t frames = ESCAPE_GUARD_FRAMES;
