...
#ifdef NETCHESSZX_SPECTRANEXT
__at(NETCHESSZX_LOWRAM_UART_RING_ADDR) char line_buf[SPECTRUM_NET_LINE_MAX];
#ifdef NETCHESSZX_FIXED_LOW_RAM
__at(NETCHESSZX_LOWRAM_LAST_IP_ADDR) char last_ip[NETCHESSZX_LOWRAM_LAST_IP_SIZE];
#else
...
static void direct_reset_rx_state(void)
{
    ...
    *(uint8_t *)NETCHESSZX_LOWRAM_DIRECT_PENDING_CURSOR_ADDR = 0u;
    *(uint8_t *)NETCHESSZX_LOWRAM_DIRECT_PENDING_LENGTH_ADDR = 0u;
...
static uint8_t mqtt_publish_queue_count;
static uint8_t mqtt_publish_queue_head;
static uint8_t mqtt_publish_queue_tail;
...
/* ponytail: three slots cover every retained setup topic and the measured
   broker ordering; use an incremental CONNECT FSM only if a real transcript
   exceeds this ceiling. */
#define MQTT_PUBLISH_QUEUE_COUNT 3u
#define MQTT_PUBLISH_QUEUE_SLOT_SIZE (SPECTRUM_NET_PAYLOAD_MAX + 2u)
#define MQTT_PUBLISH_QUEUE_SIZE \
    (MQTT_PUBLISH_QUEUE_COUNT * MQTT_PUBLISH_QUEUE_SLOT_SIZE)
#define MQTT_STREAM_STORAGE_SIZE (MQTT_STREAM_MAX + MQTT_PUBLISH_QUEUE_SIZE)
...
#endif
#if SPECTRUM_MQTT_SCRATCH_BASE < (MQTT_STREAM_BASE + MQTT_STREAM_STORAGE_SIZE)
#error "MQTT scratch overlaps stream or CONNECT publish queue"
#endif
#if (SPECTRUM_MQTT_SCRATCH_BASE + SPECTRUM_MQTT_PACKET_MAX) > 0x6800u
#error "MQTT scratch overlaps overlay slot"
#endif
static __at(MQTT_STREAM_BASE) uint8_t mqtt_stream[MQTT_STREAM_STORAGE_SIZE];
#define MQTT_PACKET SPECTRUM_MQTT_PACKET_SCRATCH
#define MQTT_STREAM mqtt_stream
#define mqtt_discarding (mqtt_stream_active & MQTT_STREAM_DISCARDING)
#define MQTT_PUBLISH_QUEUE (mqtt_stream + MQTT_STREAM_MAX)
...
static uint8_t mqtt_drain_uart_budget(uint8_t budget) __z88dk_fastcall
{
    ...
    if (!mqtt_stream_active) return 0u;
    flags = spxn_poll();
    if (flags < 0 || (flags & SPXN_POLLNVAL) != 0) {
        mqtt_abort_stream_mode();
        return 2u;
    }
    if ((flags & SPXN_POLLIN) != 0) {
        if (mqtt_discarding) {
            if (budget > mqtt_stream_len) budget = mqtt_stream_len;
        } else if (mqtt_stream_len >= MQTT_STREAM_MAX) {
            return 2u;
        } else if (budget > (uint8_t)(MQTT_STREAM_MAX - mqtt_stream_len)) {
            budget = (uint8_t)(MQTT_STREAM_MAX - mqtt_stream_len);
        }
        got = spxn_recv(
            MQTT_STREAM + (mqtt_discarding ? 0u : mqtt_stream_len), budget);
        if (got <= 0) {
            mqtt_abort_stream_mode();
            return 2u;
        }
        if (mqtt_discarding) {
            ...
            if (mqtt_stream_len == 0u) mqtt_stream_active = MQTT_STREAM_ACTIVE;
        } else {
            mqtt_stream_len = (uint8_t)(mqtt_stream_len + (uint8_t)got);
        }
        return 1u;
    }
    if ((flags & SPXN_POLLHUP) != 0) {
        mqtt_abort_stream_mode();
        return 2u;
    }
    return 0u;
...
static int16_t mqtt_take_stream_packet(void)
{
    ...
    /* During discard, len is remaining body bytes, not buffered data. */
    if (mqtt_discarding) return 0;
    ...
    if (remaining > (uint8_t)(SPECTRUM_MQTT_PACKET_MAX - header_len)) {
        ...
        if (discard < remaining) {
            ...
            mqtt_stream_active = MQTT_STREAM_ACTIVE | MQTT_STREAM_DISCARDING;
        } else {
...
static int16_t mqtt_read_stream_packet(void)
{
    ...
    filled = mqtt_fill_stream(WAIT_POLL);
#ifdef NETCHESSZX_NEXT
    if (spectrum_uart_error()) {
        return -2;
    }
...
static uint8_t mqtt_queue_publish_payload(const uint8_t *packet, uint8_t len)
{
    ...
    if (mqtt_publish_queue_count >= MQTT_PUBLISH_QUEUE_COUNT) {
        return 0u;
    }
    slot = MQTT_PUBLISH_QUEUE + mqtt_publish_queue_tail;
    ...
    slot[1u] = flags;
    mqtt_publish_queue_tail = (uint8_t)(mqtt_publish_queue_tail +
                                        MQTT_PUBLISH_QUEUE_SLOT_SIZE);
    if (mqtt_publish_queue_tail == MQTT_PUBLISH_QUEUE_SIZE) {
        mqtt_publish_queue_tail = 0u;
    }
    ++mqtt_publish_queue_count;
    return 1u;
}
...
static uint8_t mqtt_stash_packet(const uint8_t *packet, uint8_t len)
{
    ...
    if (type == SPECTRUM_MQTT_PUBLISH) {
        return mqtt_queue_publish_payload(packet, len);
    }
...
static int16_t mqtt_read_queued_payload(char *payload, uint8_t payload_cap)
{
    uint8_t *slot;
    uint8_t queued_len;

    if (mqtt_publish_queue_count == 0u) {
        return -1;
    }
    slot = MQTT_PUBLISH_QUEUE + mqtt_publish_queue_head;
    queued_len = slot[0u];
    mqtt_publish_queue_head = (uint8_t)(mqtt_publish_queue_head +
                                        MQTT_PUBLISH_QUEUE_SLOT_SIZE);
    if (mqtt_publish_queue_head == MQTT_PUBLISH_QUEUE_SIZE) {
        mqtt_publish_queue_head = 0u;
    }
    --mqtt_publish_queue_count;
    if (payload_cap == 0u || queued_len >= payload_cap) {
        return SPECTRUM_NET_MQTT_READ_TIMEOUT;
    }
    net_copy((uint8_t *)payload, slot + 2u, (uint16_t)(queued_len + 1u));
...
uint8_t mqtt_send_raw_packet(const uint8_t *packet, uint8_t len)
{
    if (mqtt_stream_active) {
#ifdef NETCHESSZX_SPECTRANEXT
        if (spxn_send_all(packet, len, 150u) == SPXN_OK) return 1u;
#else
...
#endif
    got = mqtt_read_queued_payload(payload, payload_cap);
    if (got != -1) {
        return got;
    }
