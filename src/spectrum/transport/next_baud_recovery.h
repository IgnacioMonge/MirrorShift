...
/* BRZXN runs UART_CUR at 230400. A reset/NMI can leave the ESP there while
   Mirror Shift starts its local UART at 115200. This cold preflight helper
   probes the normal rate first, then that one inherited rate. It restores the
   local UART to 115200 before the existing soft/hard fallback. */
static uint8_t spectrum_next_preflight_command_mode(void)
{
    ...
    /* Reinitialize rather than only changing the divisor: this also clears
       the software ring, UART FIFO and RX fault latch before verification. */
    spectrum_uart_init();
