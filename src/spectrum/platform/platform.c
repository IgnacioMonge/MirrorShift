...
extern uint8_t net_uart_rx_error;
...
/* RX ring under the UART wrappers. spectrum_uart_background_pump() stockpiles
   ...
   so it costs no BSS; indices are uint8_t, so 256 is the ceiling. The
   Spectranext target has no UART and omits the ring entirely. */
#ifndef NETCHESSZX_SPECTRANEXT
...
#ifdef NETCHESSZX_NEXT
    net_uart_rx_error = 0u; /* explicit discard establishes a new RX boundary */
#endif
#endif
}

#ifndef NETCHESSZX_SPECTRANEXT
/* Retry a bounded local UART TX wait. Next tests FIFO-full, not ESP
   responsiveness; successful enqueue is not remote receipt/acknowledgement. */
...
#endif
        busy = net_uart_send(c);

        /* Drain RX after every attempt: successful sends guarantee service
           between bytes, while busy retries cover prolonged TX contention. */
...
#ifdef NETCHESSZX_NEXT
    ready = net_uart_ready();
    return net_uart_rx_error ? 0u : ready;
#else
