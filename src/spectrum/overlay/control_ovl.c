    ...
    uint8_t mode)
{
    ...
    if (mode != SESSION_CONTROL_PAYLOAD) {
        uint16_t ply;
        const char *tail = netchess_mqtt_session_parse_u16_token(text, &ply);

        if (tail != 0 && tail - text <= 5 && ply != 0u &&
            (*tail == '\0' || *tail == ' ')) {
            return mode == SESSION_CONTROL_ACK
    ...
    const char *payload)
{
    ...
    if (netchess_after_prefix(payload, NETCHESS_PROTO_MOVE_PREFIX) != 0) {
        ...
        return netchess_proto_parse_move(payload,
                                         ply,
                                         sizeof(ply),
                                         move,
                                         sizeof(move),
                                         0,
                                         0u) &&
               netchess_mqtt_session_parse_u16_token(ply, &takeback_ply) != 0 &&
               takeback_ply != 0u
    ...
    tail = netchess_after_prefix(payload, NETCHESS_PROTO_CHAT_PREFIX);
    if (tail != 0 && *tail != '\0') {
