...
static uint16_t mqtt_alloc_id_ovl(void)
{
    ...
    if (mqtt_next_id == 0u) mqtt_next_id = 1u;
    return id;
}
...
uint8_t mqtt_activate_side_ovl(void)
{
    ...
    if (mqtt_activate_side_internal() && mqtt_publish_setup_ovl()) return 1u;
...
#define MQTT_PRESENCE_TEXT_CAP 10
#define MQTT_CIPSTART_PREFIX "AT+CIPSTART=\"TCP\",\""
#define MQTT_CIPSTART_SUFFIX "\","
#define MQTT_CIPSTART_PREFIX_LEN 19u
#define MQTT_CIPSTART_SUFFIX_LEN 2u
#define MQTT_PORT_TEXT_MAX 5u

#if MQTT_CIPSTART_PREFIX_LEN + NETCHESSZX_MQTT_HOST_MAX + \
        MQTT_CIPSTART_SUFFIX_LEN + MQTT_PORT_TEXT_MAX + 1u > \
    SPECTRUM_MQTT_PACKET_MAX
#error "MQTT CIPSTART command exceeds packet scratch"
#endif

#ifdef NETCHESSZX_HOST_SESSION_TEST
typedef char mqtt_presence_text_capacity_check[
    sizeof(NETCHESS_MQTT_SESSION_ONLINE_WHITE_PREFIX) + 5u <= MQTT_PRESENCE_TEXT_CAP &&
    sizeof(NETCHESS_MQTT_SESSION_ONLINE_BLACK_PREFIX) + 5u <= MQTT_PRESENCE_TEXT_CAP &&
    sizeof(NETCHESS_MQTT_SESSION_OFFLINE_WHITE_PREFIX) + 5u <= MQTT_PRESENCE_TEXT_CAP &&
    sizeof(NETCHESS_MQTT_SESSION_OFFLINE_BLACK_PREFIX) + 5u <= MQTT_PRESENCE_TEXT_CAP ? 1 : -1];
#endif
...
static uint8_t mqtt_tcp_connect_single_ovl(const char *host, uint16_t port)
{
    (void)spectrum_append_u16(
        spectrum_append_text(
            spectrum_append_text(spectrum_append_text((char *)mqtt_packet_ovl, MQTT_CIPSTART_PREFIX), host),
            MQTT_CIPSTART_SUFFIX),
        port);
...
uint8_t mqtt_connect_start_ovl(void)
{
    ...
    if (netchesszx_session_is_host() || netchesszx_host_color_ready) {
        if (!mqtt_activate_side_ovl()) return 0u;
    }
    mqtt_recovery_begin_ovl();
    return 1u;
}
...
uint8_t mqtt_activate_side_ovl(void)
{
    char offline[MQTT_PRESENCE_TEXT_CAP];
