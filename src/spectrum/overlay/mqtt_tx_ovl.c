...
{
    char *p = (char *)SPECTRUM_MQTT_PACKET_SCRATCH;
...
uint8_t mqtt_tx_clock_retry_start_ovl(void)
{
    ...
    if (!mqtt_send_at_ovl((const char *)SPECTRUM_MQTT_PACKET_SCRATCH)) {
        return 0u;
    }
...
static uint8_t next_rtc_time(void)
{
    reset_line_buf();
#asm
    push ix
...
next_rtc_done:
    pop ix
#endasm
    /* Read the mode after firmware clobbers the C argument registers. */
    return capture_msdos_time_bytes_mode((const volatile uint8_t *)line_buf,
        (uint8_t)(netchesszx_timezone == NETCHESSZX_TIME_RTC));
}
#endif
...
uint8_t mqtt_tx_sync_time_ovl(void)
{
...
#ifdef NETCHESSZX_NEXT
    netchesszx_rtc_available = next_rtc_time();
    if (netchesszx_rtc_available && timezone == NETCHESSZX_TIME_RTC) {
        return 1u;
    }
#endif
    ...
    p = numeric_end;
    (void)spectrum_append_text(p, ",\"pool.ntp.org\",\"time.google.com\"");
    if (!spectrum_net_at_cmd((const char *)SPECTRUM_MQTT_PACKET_SCRATCH,
                             WAIT_SHORT)) {
        *numeric_end = '\0';
        (void)spectrum_net_at_cmd((const char *)SPECTRUM_MQTT_PACKET_SCRATCH,
                                  WAIT_SHORT);
    }
...
uint8_t mqtt_tx_sync_time_ovl(void)
{
    return 0u; /* Spectranext dispatches time to the cold TIME overlay. */
}
