    ...
    if (phase == STATUS_PHASE_CONNECTION_SETUP) {
        ...
        if (netchesszx_transport_is_mqtt()) {
            ...
            if (netchesszx_session_is_host()) {
                host = spectrum_esp_at_last_ip();
            } else {
                host = netchesszx_direct_host;
            }
            if (host[0] == '\0') {
                host = "-";
            }
            p = status_append(p, host, end);
            p = status_append_direct_port(p, end);
        }
        p = status_append(p, " ", end);
        ...
        if (phase == STATUS_PHASE_CONNECTING) {
            ...
            const char *code = status_platform_code(platform);

            p = status_append(p, " VS ", end);
            (void)status_append(p, code, end);
        }
    }
}
