...
constexpr const char *kSaveExtension = ".msh";
...
bool parseSlotBaseName(const QString &base, int *slot, QDateTime *when)
{
    ...
    const bool reservedStamp = base.mid(2) == QStringLiteral("000000");

    if (slotValue < 1 || slotValue > SaveGameStore::kSlotCount || year < 0) {
        return false;
    }
    *slot = slotValue;
    if (reservedStamp) {
...
QString defaultDirectory()
{
    ...
    (void)dir.mkpath(QStringLiteral("MirrorShift/Saves"));
    return dir.filePath(QStringLiteral("MirrorShift/Saves"));
}
...
QVector<SaveSlotEntry> scanSlots(const QString &directory)
{
    ...
    const QStringList files = QDir(directory).entryList(
        QStringList() << QStringLiteral("*.msh"), QDir::Files);
    ...
    for (const QString &file : files) {
        ...
        entry.baseName = base;
        entry.filePath = QDir(directory).filePath(file);
        entry.when = when;
        entry.used = true;
    }
    return entries;
