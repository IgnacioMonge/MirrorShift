...
uint8_t netchesszx_asm_move_tail_ok(const char *tail) NETCHESSZX_FASTCALL;
#define proto_copy_token netchesszx_asm_proto_copy_token
#define move_syntax_ok netchesszx_asm_move_syntax_ok
#define move_tail_ok netchesszx_asm_move_tail_ok
#endif
...
/* A Mirror Shift move is a single Lattice square such as "d3"; Silence is
   resolved by the rules core and never travels on the wire. */
static uint8_t move_syntax_ok(const char *move, uint8_t len)
{
    return (uint8_t)(len == 2u && is_file_char(move[0]) &&
                     is_rank_char(move[1]));
}

static uint8_t move_tail_ok(const char *tail) NETCHESSZX_FASTCALL
{
    if (*tail++ == '\0') {
        return 1u;
    }
