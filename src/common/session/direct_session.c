                             ...
                             uint8_t close_link)
{
    ...
    uint8_t reason = SESSION_END_REASON_TRANSPORT_LOST;

    if (state->pending_control == SESSION_REQUEST_BYE) {
        reason = state->pending_origin == DIRECT_ORIGIN_LOCAL
    ...
    if (!session_emit_end(actions, &count, reason)) {
        return 0u;
    }
    session_reset(state);
    return count;
                                    ...
                                    uint8_t *count)
{
    ...
    prefix_len = session_text_length(prefix);
    value_len = session_text_length(value);
    if (tx_scratch == 0 || (uint16_t)prefix_len + (uint16_t)value_len >= tx_capacity) {
        return 0u;
    }
    memcpy(tx_scratch, prefix, prefix_len);
    memcpy(tx_scratch + prefix_len, value, (size_t)value_len + 1u);
    return direct_send_buffer(state, tx_scratch, tx_capacity, actions, count,
...
{
    char ply_text[6];
    char move[SESSION_MOVE_TEXT_LEN + 1u];
    uint16_t ply;
                                                ...
                                                SessionAction *actions)
{
    ...
    if (session_text_equal(payload, length, NETCHESS_PROTO_CANCEL_RESET)) {
        ...
        return direct_send_text(state, "NACK RESET",
                                tx_scratch, tx_capacity, actions, &count,
                                state->active_link,
                                DIRECT_TX_NACK_RESET_BUSY)
                   ? count : 0u;
                                              ...
                                              SessionAction *actions)
{
    ...
    if (netchess_proto_matches_token((const char *)payload,
                                    "NACK GAME START") &&
        state->pending_control == SESSION_REQUEST_START &&
        state->pending_origin == DIRECT_ORIGIN_LOCAL &&
        state->pending_request_id == 0u) {
    ...
    if (netchess_proto_matches_token((const char *)payload, "NACK RESET") &&
        state->pending_control == SESSION_REQUEST_RESET &&
         state->pending_origin == DIRECT_ORIGIN_LOCAL &&
        state->pending_request_id == 0u) {
...
static uint8_t direct_payload_credits_liveness(const SessionState *state,
                                               const uint8_t *payload,
                                               uint8_t length)
{
    char value_text[6];
    ...
    if (session_text_equal(payload, length, "PING") ||
        session_text_equal(payload, length, "ACK PING") ||
        session_text_equal(payload, length, "BUSY") ||
        session_text_equal(payload, length, "BYE") ||
        session_text_equal(payload, length, "RESET") ||
        session_text_equal(payload, length, "RESIGN") ||
        session_text_equal(payload, length, NETCHESS_PROTO_CANCEL_RESET) ||
        session_text_equal(payload, length, "RQ") ||
        session_text_equal(payload, length, "RY") ||
        session_text_equal(payload, length, "RN") ||
        session_text_equal(payload, length, "RA") ||
        session_text_equal(payload, length, "ACK GAME START") ||
        session_text_equal(payload, length, "ACK RESET") ||
        session_text_equal(payload, length, "ACK RESIGN") ||
        netchess_proto_matches_token((const char *)payload,
                                     "NACK GAME START") ||
        netchess_proto_matches_token((const char *)payload, "NACK RESET")) {
        return 1u;
    }
    ...
    if (netchess_proto_parse_move((const char *)payload,
                                  value_text, sizeof(value_text),
                                  move, sizeof(move), 0, 0u) &&
        session_parse_u16(value_text) != 0u) {
        return 1u;
    }
    ...
    if ((state->config.role == SESSION_ROLE_HOST &&
         netchess_direct_parse_guest_hello((const char *)payload)) ||
        (state->config.role == SESSION_ROLE_GUEST &&
         netchess_direct_parse_host_hello((const char *)payload, &owner))) {
        return 1u;
    }
    if ((netchess_proto_parse_ack((const char *)payload,
                                  value_text, sizeof(value_text),
                                  detail, sizeof(detail)) ||
         netchess_proto_parse_nack((const char *)payload,
                                   value_text, sizeof(value_text),
                                   detail, sizeof(detail))) &&
        session_parse_u16(value_text) != 0u) {
        return 1u;
    ...
    takeback = netchess_after_prefix((const char *)payload, "TAKEBACK ");
    if (takeback != 0 && session_parse_u16(takeback) != 0u) {
        return 1u;
    }
    return (uint8_t)(length == 35u &&
                     (session_text_prefix(payload, length, "RS00 ") ||
                      session_text_prefix(payload, length, "RS01 ")));
}
                                ...
                                SessionAction *actions)
{
    ...
    if (direct_payload_credits_liveness(state, payload, length)) {
                                   ...
                                   SessionAction *actions)
{
    ...
    case SESSION_REQUEST_MOVE:
        if (state->phase != SESSION_PHASE_ACTIVE ||
            ...
            event->data.local.length != SESSION_MOVE_TEXT_LEN) {
            return 0u;
        }
        if (!session_emit_timer_cancel(state, actions, &count,
                                      SESSION_TIMER_LIVENESS)) {
