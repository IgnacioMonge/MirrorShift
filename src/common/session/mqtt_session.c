                                    ...
                                    uint8_t *count)
{
    ...
    value_len = session_text_length(value_text);
    if ((uint16_t)prefix_len + (uint16_t)value_len >= tx_capacity) {
        return 0u;
    }
    memcpy(tx_scratch, prefix, prefix_len);
    memcpy(tx_scratch + prefix_len, value_text, (size_t)value_len + 1u);
    return mqtt_send_buffer(state,
                            tx_scratch,
                            tx_capacity,
                            actions,
                           ...
                           uint8_t close_link)
{
    ...
    if (state->pending_control == SESSION_REQUEST_BYE &&
        ...
        reason = SESSION_END_REASON_LOCAL_BYE;
    }
                                         ...
                                         SessionAction *actions)
{
    ...
    control = netchess_proto_matches_token((const char *)payload,
                                           "NACK RESET")
                  ? SESSION_REQUEST_RESET
                  : 0u;
                                ...
                                SessionAction *actions)
{
    ...
    if (state->pending_control == SESSION_REQUEST_MOVE &&
        ...
        if (needed > SESSION_ACTION_CAPACITY) {
            return 0u;
        }
        if (!session_emit_timer_cancel(state,
                                    actions,
                          ...
                          uint8_t count)
{
    ...
    result = mqtt_tx_ok_control(state, tx_kind, tx_scratch,
                                tx_capacity, actions, count);
    if (result != MQTT_TX_UNHANDLED) {
        return result;
    }
