...
uint8_t session_text_equal(const uint8_t *payload,
                           ...
                           const char *text)
{
    uint8_t n = session_text_length(text);

    return (uint8_t)(n == length && memcmp(payload, text, n) == 0);
}
...
uint8_t session_text_prefix(const uint8_t *payload,
                            ...
                            const char *prefix)
{
    uint8_t n = session_text_length(prefix);

    return (uint8_t)(n <= length && memcmp(payload, prefix, n) == 0);
}
...
uint8_t session_slice_valid(const uint8_t *payload, uint8_t length)
{
    ...
    return (uint8_t)(memchr(payload, 0, length) == 0);
}
                             ...
                             uint8_t status)
{
    ...
    actions[*count].data.session.reason = SESSION_END_REASON_NONE;
    ++*count;
    return 1u;
}
                         ...
                         uint8_t reason)
{
    ...
    actions[(uint8_t)(*count - 1u)].data.session.reason = reason;
    return 1u;
}
                                      ...
                                      uint8_t chunk)
{
    ...
    return (uint8_t)(memcmp(workspace->restore + offset, payload + 5u, 30u) == 0);
}
                    ...
                    uint8_t close_link)
{
    ...
    actions[count].data.session.reason = SESSION_END_REASON_TRANSPORT_LOST;
    ++count;
    session_reset(state);
    return count;
}
