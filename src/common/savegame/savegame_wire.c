...
static int save_piece_to_nibble(char piece, uint8_t *out)
{
    ...
    case 'A': *out = 0x1u; return NETCHESSZX_SAVE_OK;
    case 'B': *out = 0x2u; return NETCHESSZX_SAVE_OK;
    default: break;
    }
...
static int save_nibble_to_piece(uint8_t nibble, char *out)
{
    ...
    case 0x1u: *out = 'A'; return NETCHESSZX_SAVE_OK;
    case 0x2u: *out = 'B'; return NETCHESSZX_SAVE_OK;
    default: break;
    }
    return NETCHESSZX_SAVE_ERR_BOARD;
}
...
int netchesszx_save_state_validate(const netchesszx_save_state_t *state)
{
    ...
    if (state->host_color > NETCHESSZX_SAVE_HOST_BLACK ||
        state->side > NETCHESSZX_SAVE_SIDE_BLACK || state->over > 1u) {
        return NETCHESSZX_SAVE_ERR_FIELD;
    }
    if ((state->flags & (uint8_t)~NETCHESSZX_SAVE_FLAGS_MASK) != 0u ||
        (state->view_flags & (uint8_t)~NETCHESSZX_SAVE_VIEW_MASK) != 0u) {
        return NETCHESSZX_SAVE_ERR_FIELD;
    }
    if (((state->flags & NETCHESSZX_SAVE_FLAG_GAME_OVER) != 0u) !=
            (state->over != 0u) ||
        ((state->flags & NETCHESSZX_SAVE_FLAG_ACTIVE) != 0u &&
         state->over != 0u)) {
        return NETCHESSZX_SAVE_ERR_FIELD;
    }
    ...
    for (i = 0u; i < 64u; ++i) {
        if (!save_piece_ok(state->cells[i])) {
            return NETCHESSZX_SAVE_ERR_BOARD;
        }
    }
    return NETCHESSZX_SAVE_OK;
                                ...
                                size_t len)
{
    ...
    meta = wire[32];
    if ((meta & (uint8_t)~0x47u) != 0u || wire[33] != 0u) {
        return NETCHESSZX_SAVE_ERR_FIELD;
    }
    state->side = (uint8_t)(meta & 1u);
    state->host_color = (uint8_t)((meta >> 1) & 1u);
    state->over = (uint8_t)((meta >> 2) & 1u);
