...
; the eighteen 4K overlay pages; slot 3 selects them directly thereafter.
;
; Screen is asm/spectrum/screen.asm with NETCHESSZX_NEXT_GFX: board squares,
; pieces and move markers are Next hardware sprites. UI rendering executes from the permanent slot-1 extension;
; native sprite/palette setup remains in the inherited resident service.
...
PUBLIC _netchesszx_piece_set_finalize
...
EXTERN next_graphics_bank_final
...
next_overlay_page_count EQU 18
...
; Expansion is the only owner during boot and must finish at zero before main.
; Reuse that dead byte for the runtime non-nesting guard: no resident BSS cost.
ovl_call_active EQU next_expand_left
...
; fastcall: piece-set index in L. Upload the native morph/capture patterns
; and restore the canonical 64-byte Reversi A/B masks.
...
; Commit the stable pair only after BOARD has completed the setup morph.
_netchesszx_piece_set_finalize:
    ld hl, next_graphics_bank_final
    jp next_graphics_bank_call
...
; Z80N fixed-register writes preserve registers, flags and IFF.
next_map_slot0:
    DEFB 0xed, 0x92, next_mmu_slot0
    ret

next_map_slot2:
    DEFB 0xed, 0x92, next_mmu_slot2
    ret
