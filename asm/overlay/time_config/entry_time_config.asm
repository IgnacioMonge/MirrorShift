...
EXTERN _spectrum_net_runtime_set_fat_stamp
...
; Cart builds have no esxDOS/NextZXOS RTC. Keep this cold entry safe when
; reached directly before the resident dispatcher rejects the probe.
tc_rtc_probe:
    ld l, 0
    ret
ELSE
...
tc_rtc_ok:
    ...
    push de
    push bc
    call _spectrum_net_runtime_set_fat_stamp
    pop bc
    pop bc
    jp tc_ret1
tc_rtc_no:
...
tc_rtc_pcf_validate:
    ...
    ld a, (hl)
    bit 7, a
    jp nz, tc_rtc_pcf_fail
    and 0x7f
    call tc_rtc_bcd_to_bin
    ret c
    cp 60
    jp nc, tc_rtc_pcf_fail
    ld (hl), a
    ...
    ; Pack validated PCF8563 Y/M/D and H/M/S into the same FAT fields.
    add a, 20
    add a, a
    ld b, a
    ld a, (CTX + 5)
