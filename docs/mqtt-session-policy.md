...
- The Spectrum MQTT build is now a single package:
  `MQTT/MIRSHIFT.TAP` plus `MQTT/MIRSHIFT.OVL`.
...
Either linked peer may initiate MQTT RESTORE. It uses the directional game topic
...
is transmitted. The initiator sends `RQ`; the receiver accepts with `RY` or rejects
with `RN`. After `RY`, the initiator sends `RS00` then `RS01`. The receiver applies only
the complete snapshot and answers `RA` on success or `RN` on failure.

The control timer bounds every wait. The initiator retries `RQ` while awaiting `RY`
and retries the ordered chunk pair while awaiting `RA`. The receiver re-sends `RY`
for a duplicate accepted `RQ`, waits a bounded time for incomplete chunks, and
sends `RN` when that receive window expires. An initiator may cancel with `RN` only
