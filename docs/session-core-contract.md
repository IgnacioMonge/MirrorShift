...
`MACH` is the deliberate adapter-level exception: optional presentation
metadata parsed with the common protocol helper only after link/route checks.
It never enters the reducer, credits liveness, or changes session/game state;
invalid or retained announcements are ignored.
...
holds independent fields for the pending two-byte placement, rejected-placement
identity, local chat text, and 60-byte restore payload across later TX-result
events. A local CHAT or MOVE therefore cannot overwrite an applied RESTORE's
duplicate chunks. It replaces equivalent target-local pending buffers; it is
not additive scratch.
...
- `EV_RX`: link id, route, retained/live flags, pointer, and `uint8_t` payload
  length.
- `EV_LOCAL_REQUEST`: START, MOVE, CHAT, RESET, RESIGN, TAKEBACK, BYE, or
...
- the caller supplies at least three writable bytes;
- return 1 produces one NUL-terminated two-byte Lattice coordinate;
- return 0 leaves the destination contents undefined and may have written a
  valid prefix, but never writes beyond the three-byte destination;
...
three-byte stack local and calls `send_local_move()` only on success; its failure
...
the 66-byte Mirror Shift state at `0x5f60`, followed by its 66-byte takeback
state at `0x5fa2`, 11 bytes of last-Alignment detail at `0x5fe4`, eight hint
row masks at `0x5ff0`, the overlay context at `0x5ff8`, and overlay code at
`0x6800`. The target wrapper passes the stack destination through the context;
it does not alias rules state.
...
for bounds safety; the three-byte branch bound and success-before-use rule do.
...
- `ACT_SESSION_CHANGED`: READY, BUSY, STARTED, or ENDED. ENDED carries a typed
  cause: local BYE, remote BYE, or unexpected transport loss; presentation
  must not infer that cause by parsing a wire payload.
...
The compact Spectrum implementation uses the inherited five-byte undo ABI as
a validity handle for a prevalidated 66-byte rules snapshot in fixed low RAM.
It restores that snapshot through a void operation; after the request guards
pass, that apply has no runtime failure source. Its parity runner therefore marks the
...
RESET during an active game is
NACKed BUSY; and either peer receiving RQ prompts for approval. PING is
...
duplicate latch compares the complete two-byte placement as well as its ply:
an exact replay repeats the prior result, while a different placement at that
ply is validated as a fresh request. A busy GAME START is rejected as
`NACK GAME START BUSY` without disturbing the pending operation.
...
450-tick windows without peer data. Any valid, fully parsed peer payload resets the miss
...
loss. SpectraNext DIRECT advances one frame per empty poll, so its PAL threshold
is 150 polls at 50 Hz; the cartridge target must not probe ZX Spectrum Next
registers. The compact Next runtime instead selects 75 two-frame
polling quanta at 50 Hz or 90 at 60 Hz from the actual video-timing mode. The
same production timing table and `ping.c` are compiled and tested for PAL
Classic, PAL SpectraNext, and both Next refresh rates.

A local TAKEBACK confirmation is not a pending wire operation until accepted
and sent. A valid opponent MOVE takes precedence over that unsent confirmation:
apply and acknowledge the move, then discard the obsolete request.

An incoming TAKEBACK awaiting the user's decision is not an outgoing request.
It must neither activate the local retransmission/timeout budget nor emit
TAKEBACK back to its requester. Generic ACK/NACK replies consume only a locally
sent TAKEBACK; unrelated replies leave the incoming prompt and its ply intact.
Only explicit Y/N answers that prompt; declining allows a fresh same-ply request.

Only one control operation may be pending. While a local MOVE or TAKEBACK
is awaiting its peer result, a fresh RESET which would create a second
operation is NACKed BUSY. This deliberately
...
CHAT is not a control operation and remains available while START, RESET,
...
game has not started. After a RESTORE is applied, its exact-chunk duplicate
cache remains valid across local CHAT and MOVE transmissions.
...
RESET does not retain a duplicate latch after its reply has
...
observable interoperability action. The compact FSM derives the poll count
from the actual empty-poll frame cost and refresh rate, rounding up to at least
2500 ms. The canonical reducer and PC v1.0 use 125 ticks (2500 ms). MQTT parity
normalizes only the timer SET/CANCEL bookkeeping; the timeout input, resulting
wire retransmission, TX guard and liveness effects remain observable.
...
- Compact Classic and SpectraNext builds are PAL 50 Hz. SpectraNext DIRECT
  counts one frame per empty poll while Classic DIRECT and all compact MQTT
  paths count two.
...
application peer deadline reports `SESSION_END_REASON_REMOTE_BYE`, discards the
game, and returns the reducer to `SESSION_PHASE_HANDSHAKE` while preserving
`link_up`, `active_link`, and the broker transport. A replacement peer is
therefore accepted without another `EV_LINK_UP`. Before that logical end, a
current room HOST with an established peer publishes retained
...
The binary record is Mirror Shift save version 2 as defined by
`docs/wire-contract.md`. Version 1 chess records are deliberately incompatible:
the domain decoder reports failure, the reducer uses the existing `RN` path,
and the current game remains unchanged.
