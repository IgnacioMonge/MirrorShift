#!/usr/bin/env python3
"""Generate resident symbol definitions for Mirror Shift overlays.
...
REQUIRED_SYMBOLS = [
    ...
    "_spectrum_gui_active_coord_valid",
    "_spectrum_gui_active_coord_row",
    "_spectrum_gui_active_coord_col",
    ...
    "_spectrum_net_runtime_fat_elapsed_days",
    ...
    "_spectrum_board_is_legal_index",
    "_spectrum_board_check_state",
    "_ms_rules_side",
    "_ms_rules_cells_side_has_moves",
    "_ms_rules_is_over",
    "_last_ply_seen",
    "_move_line_count",
    "_chat_line_count",
...
# Backend-only imports are emitted when present, but are not part of the
# shared resident ABI manifest used by Classic and Spectrum Next.
BACKEND_OPTIONAL_SYMBOLS = [
    "_spectrum_gui_shift_clock",
    "_spxn_status",
    "_spxn_close",
    "_spxn_resolve",
    "_spxn_connect",
...
def main(argv):
    ...
    print(";; Resident symbols for Mirror Shift overlay linking")
    print()
    emitted = set()
    ...
    for name in LINK_ONLY_SYMBOLS:
        if name in symbols:
            print(f"PUBLIC {name}")
            print(f"DEFC {name} = ${symbols[name]:04X}")
            emitted.add(name)
    for name in OPTIONAL_SYMBOLS + BACKEND_OPTIONAL_SYMBOLS:
        if name in symbols:
            print(f"PUBLIC {name}")
            print(f"DEFC {name} = ${symbols[name]:04X}")
            emitted.add(name)
