#!/usr/bin/env python3
"""Generate and compare Mirror Shift size reports."""
...
def make_report(args: argparse.Namespace) -> dict[str, object]:
    ...
    low_code_bytes = 0
    if symbols.get("_overlay_code_slot") == 0x2000:
        low_code_bytes = (symbols["_spxn_overlay_loader_end"] -
                          symbols["_spxn_overlay_loader_start"])
        resident_bytes += low_code_bytes
        bss_end = max(symbols["__BSS_END_tail"], symbols["__bss_compiler_tail"],
                      symbols["__bss_user_tail"])
        bss_bytes = (symbols["__bss_compiler_tail"] - bss_head +
                     symbols["__bss_user_tail"] - symbols["__bss_user_head"] +
                     symbols["__BSS_UNINITIALIZED_tail"] - symbols["__BSS_UNINITIALIZED_head"])
    ...
    report: dict[str, object] = {
        ...
        "code": {
            ...
            "low_code_bytes": low_code_bytes,
            "trimmed_bytes": resident_bytes,
            "bss_trim_saved_bytes": (code_full + low_code_bytes - resident_bytes)
...
def check_hard_limits(report: dict[str, object]) -> list[str]:
    minimum = 3072 if report.get("markers", {}).get("_overlay_code_slot") == "0x6000" else MIN_SP_GAP
    gap = metric(report, ("memory", "register_sp_gap_bytes"))
    if gap is None:
        return ["SP_GAP missing in current size report"]
    if gap < minimum:
        return [
            f"SP_GAP {gap} bytes is below hard floor {minimum} bytes"
        ]
    if gap < MIN_SP_GAP_WARN:
        print(f"[WARN] SP_GAP {gap} bytes is below warning threshold {MIN_SP_GAP_WARN} bytes")
    print(f"[OK] SP_GAP hard floor: {gap} >= {minimum} bytes")
    return []


def check_policy_limits(
    report: dict[str, object], policy_path: Path, target: str
) -> list[str]:
    with policy_path.open("rb") as handle:
        policy = tomllib.load(handle)
    try:
        limits = policy["targets"][target]
        resident = report["code"]["resident_bytes"]
        gap = report["memory"]["register_sp_gap_bytes"]
    except KeyError as exc:
        return [f"size policy missing {exc} for target {target}"]
    errors = []
    if resident is None or resident > int(limits["resident_ceiling"]):
        errors.append(
            f"RESIDENT {resident} exceeds {target} ceiling "
            f"{limits['resident_ceiling']}"
        )
    if gap is None or gap < int(limits["stack_gap_floor"]):
        errors.append(
            f"SP_GAP {gap} is below {target} floor {limits['stack_gap_floor']}"
        )
    return errors
...
def main(argv: list[str]) -> int:
    ...
    if hard_limit_errors:
        ...
        return 1
    if (args.policy is None) != (args.policy_target is None):
        print("[ERR] --policy and --policy-target must be used together", file=sys.stderr)
        return 1
    if args.policy is not None:
        policy_errors = check_policy_limits(report, args.policy, args.policy_target)
        if policy_errors:
            for error in policy_errors:
                print(f"[ERR] {error}", file=sys.stderr)
            return 1
        print(f"[OK] {args.policy_target} size policy limits")
