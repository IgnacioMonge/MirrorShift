...
HEIGHT = 192
TEXT_LINES = [
    ("MIRROR SHIFT FOR SPECTRUM NEXT", (255, 255, 255)),
    ("(C) 2026 M.I. MONGE GARCIA", (255, 255, 255)),
    ("GITHUB.COM/IGNACIOMONGE/MIRRORSHIFT", (255, 255, 0)),
...
def draw_icon(
    px: bytearray, colors: list[tuple[int, int, int]], source: Path
) -> None:
    icon = Image.open(source).convert("RGB").resize(
    ...
    for y in range(icon.height):
        for x in range(icon.width):
            rgb = icon.getpixel((x, y))
            index = cache.get(rgb)
            if index is None:
                index = nearest_index(rgb, colors)
                cache[rgb] = index
            px[(top + y) * WIDTH + left + x] = index
...
def main() -> int:
    ...
    background = nearest_index((12, 15, 26), colors)
    px = bytearray((background,)) * (WIDTH * HEIGHT)
    ...
    draw_icon(
        px,
        colors,
        Path(__file__).resolve().parents[1] / "client/mirrorshift.png",
    )
    draw_text(
        px,
        font,
        118,
        "MIRROR SHIFT",
        nearest_index((232, 238, 252), colors),
        shadow,
        scale=2,
    )
