#!/usr/bin/env python3
"""Generate and compare the Mirror Shift overlay ABI manifest."""
...
def parse_constants(
    ...
    if target is not None:
        source = select_source(source, target)
...
def make_manifest(
    ...
    target = {0x6000: "next", 0x2000: "spectranext"}.get(symbols.get("_overlay_code_slot"), "classic")
    parse_constants(lowram_header, "NETCHESSZX_LOWRAM_", context_values, target)
    ...
    return {
        ...
        "overlay_constants": parse_constants(overlay_header, "SPECTRUM_OVL_", target=target),
