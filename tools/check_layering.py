...
UI_BOARD_VIEW_TOKEN = "NETCHESSZX_LOWRAM_BOARD_STATE_ADDR"
...
def load_lowram_map(root: Path, target: str = "classic") -> dict[str, int]:
    constants: dict[str, int] = {}
    for rel_path in (LOWRAM_MAP_OWNER, OVERLAY_CONTEXT_OWNER):
        path = root / rel_path
        if not path.exists():
            continue
        for line in select_source(path.read_text(encoding="utf-8", errors="replace"), target).splitlines():
...
def find_violations(
    ...
    next_lowram_constants = load_lowram_map(root, "next")
        ...
        for line_no, line in enumerate(text, 1):
            ...
            if equ_match:
                ...
                if expected_name is not None:
                    ...
                    constants = next_lowram_constants if rel.startswith("asm/next/") else lowram_constants
                    expected = constants.get(expected_name)
