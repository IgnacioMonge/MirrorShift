...
def validate_image(
    ...
    ram_end = max(required_symbol(symbols, "__BSS_END_tail"),
    ...
    load_org = org if load_org is None else load_org
    prefix_size = org - load_org
    if prefix_size < 0 or len(low_code) > prefix_size:
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--pack", action="store_true", help="Package split cartridge sections before validating")
    args = parser.parse_args(argv)

    try:
        if args.pack:
            if args.low_code_bin is None or args.load_org is None:
                raise TapImageError("--pack requires --low-code-bin and --load-org")
            low_code = args.low_code_bin.read_bytes()
            padding = args.org - args.load_org - len(low_code)
            if padding < 0:
                raise TapImageError("low CODE section reaches resident ORG")
            combined = args.code_bin.with_name(args.code_bin.stem + "_PACKED.bin")
            combined.write_bytes(low_code + bytes(padding) + args.code_bin.read_bytes())
            subprocess.run(["z88dk-appmake", "+zx", "-b", str(combined),
                            "--org", str(args.load_org), "--clearaddr", str(args.load_org - 1),
                            "--usraddr", str(args.org), "-o", str(args.tap)], check=True)
