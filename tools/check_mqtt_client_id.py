...
def main() -> None:
    ...
    if 'DEFB 0, 4, "MQTT", 4, $02' not in asm or not all(
        token in asm
        for token in (
            "ld a, $06",
        ...
        fail("Spectrum CONNECT role flags or dynamic lengths changed")
    if "jr nz, mqtt_conn_finish" not in asm:
        fail("Spectrum guest CONNECT must omit the host will")
