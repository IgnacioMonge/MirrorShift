...
static void make_valid(uint8_t *record)
{
    ...
    memcpy(record + NETCHESSZX_APP_CONFIG_ROOM_OFF, "MS12AB", 7u);
...
int main(void)
{
    ...
    make_valid(record);
    record[NETCHESSZX_APP_CONFIG_FLAGS_OFF] |=
        (3u << NETCHESSZX_APP_CONFIG_FLAG_SET_SHIFT);
    refresh_crc(record);
    assert(!netchesszx_app_config_validate(record));
    make_valid(record);
    record[NETCHESSZX_APP_CONFIG_THEME_OFF] = NETCHESSZX_BOARD_THEME_COUNT;
    refresh_crc(record);
    assert(!netchesszx_app_config_validate(record));
    ...
    assert(!netchesszx_app_config_validate(record));
    make_valid(record);
    memcpy(record + NETCHESSZX_APP_CONFIG_ROOM_OFF, "MS12AG", 7u);
    refresh_crc(record);
    assert(!netchesszx_app_config_validate(record));
    make_valid(record);
    record[NETCHESSZX_APP_CONFIG_ROOM_OFF + 6u] = 'X';
    refresh_crc(record);
    assert(!netchesszx_app_config_validate(record));
    ...
    memcpy(record + NETCHESSZX_APP_CONFIG_HOST_OFF, "255.255.255.255", 16u);
    refresh_crc(record);
    assert(netchesszx_app_config_validate(record));
    ...
    puts("app config format tests ok");
    return 0;
}
