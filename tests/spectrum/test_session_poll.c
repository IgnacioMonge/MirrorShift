...
static void test_payload_event(void)
{
    ...
    ping.idle_ticks = 42u;
    ping.misses = 3u;
    stub_payload = "MOVE 1 d3 x4 junk";
    require_u8("malformed move poll",
               netchesszx_session_poll(&ping, payload, sizeof(payload), &out),
               NETCHESSZX_SESSION_POLL_EVENT);
    require_u8("malformed move tag",
               (uint8_t)out.event,
               NETCHESSZX_SESSION_EVENT_UNKNOWN);
    require_u8("malformed move idle kept", ping.idle_ticks, 42u);
    require_u8("malformed move misses kept", ping.misses, 3u);

    stub_payload = "MACH PC";
    require_u8("direct machine identity poll",
               netchesszx_session_poll(&ping, payload, sizeof(payload), &out),
               NETCHESSZX_SESSION_POLL_EVENT);
    require_u8("direct machine identity tag",
               (uint8_t)out.event,
               NETCHESSZX_SESSION_EVENT_MACH_PC);
    require_u8("direct machine identity idle kept", ping.idle_ticks, 42u);
    require_u8("direct machine identity misses kept", ping.misses, 3u);
}
...
static void test_mqtt_machine_identity_route_and_retained(void)
{
    char payload[SPECTRUM_LINK_PAYLOAD_MAX];
    netchesszx_session_ping_t ping;
    netchesszx_session_poll_result_t out;
    ...
    require_u8("retained MACH poll",
               netchesszx_session_poll(&ping, payload, sizeof(payload), &out),
               NETCHESSZX_SESSION_POLL_NONE);
    require_u8("retained MACH ignored",
               (uint8_t)out.event,
               NETCHESSZX_SESSION_EVENT_UNKNOWN);

    stub_payload_flags = 0u;
    require_u8("wrong-route MACH poll",
               netchesszx_session_poll(&ping, payload, sizeof(payload), &out),
               NETCHESSZX_SESSION_POLL_NONE);
    require_u8("wrong-route MACH ignored",
               (uint8_t)out.event,
               NETCHESSZX_SESSION_EVENT_UNKNOWN);

    stub_payload_flags = SPECTRUM_LINK_PAYLOAD_GAME_ROUTE;
    require_u8("live game-route MACH poll",
               netchesszx_session_poll(&ping, payload, sizeof(payload), &out),
               NETCHESSZX_SESSION_POLL_EVENT);
    require_u8("live game-route MACH delivered",
               (uint8_t)out.event,
               NETCHESSZX_SESSION_EVENT_MACH_PC);
}
...
static void test_direct_failed_ping_disconnects(void)
{
    ...
    for (i = 0u;
         i < (uint16_t)(NETCHESSZX_SESSION_DIRECT_3S_POLLS - 1u);
         ++i) {
...
static void test_direct_host_timeout_is_passive(void)
{
    ...
    for (i = 0u;
         i < (uint16_t)(NETCHESSZX_SESSION_DIRECT_3S_POLLS * 6u - 1u);
         ++i) {
...
static void test_removed_draw_has_no_liveness(void)
{
    static const char *removed_draw[] = {
        "DRAW", "ACK DRAW", "NACK DRAW", "NACK DRAW BUSY", "CANCEL DRAW"
    };
    static const char *invalid[] = {
        "ACK RESETX", "NACK RESETX", "NACK RESIGN", "ACK 0", "NACK 65536",
        "DRAW", "ACK DRAW", "NACK DRAW", "NACK DRAW BUSY", "CANCEL DRAW"
    };
    char payload[SPECTRUM_LINK_PAYLOAD_MAX];
    netchesszx_session_ping_t ping;
    ...
    for (i = 0u; i < sizeof(invalid) / sizeof(invalid[0]); ++i) {
        stub_payload = invalid[i];
        (void)netchesszx_session_poll(&ping, payload, sizeof(payload), &out);
        require_u8(invalid[i], out.event, NETCHESSZX_SESSION_EVENT_UNKNOWN);
        require_u8("invalid control keeps misses", ping.misses, 2u);
    }
    ...
    netchesszx_session_configure(NETCHESSZX_SESSION_ROLE_HOST,
                                 ...
                                 NETCHESSZX_COLOR_WHITE);
    netchesszx_session_peer_mark_ready();
    for (i = 0u; i < sizeof(removed_draw) / sizeof(removed_draw[0]); ++i) {
        seed_ping(&ping, 2u);
        stub_payload = removed_draw[i];
        (void)netchesszx_session_poll(&ping, payload, sizeof(payload), &out);
        require_u8(removed_draw[i], out.event, NETCHESSZX_SESSION_EVENT_UNKNOWN);
        require_u8("removed MQTT draw keeps misses", ping.misses, 2u);
    }
}

int main(void)
{
    test_removed_draw_has_no_liveness();
    test_payload_event();
    test_retained_side_effect_event_ignored();
    test_mqtt_machine_identity_route_and_retained();
