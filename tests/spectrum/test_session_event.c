...
static void test_common_protocol_events(void)
{
    ...
    expect_event("NACK DRAW BUSY", 0u, 0u,
                 NETCHESSZX_SESSION_EVENT_UNKNOWN,
                 "removed draw nack reason ignored");
    expect_event("NACK RESIGN", 0u, 0u,
                 NETCHESSZX_SESSION_EVENT_UNKNOWN,
                 "unsupported nack resign");
    {
        static const char *invalid[] = {
            "ACK", "ACK ", "ACK RESETX", "ACK DRAW BUSY", "ACK nonsense",
            "NACK ", "NACK DRAWX", "NACK RESIGN BUSY", "ACK 0", "NACK 0",
            "ACK 65536", "NACK 1x", "ACK -1", "NACK 000001"
        };
        unsigned i;
        for (i = 0u; i < sizeof(invalid) / sizeof(invalid[0]); ++i) {
            expect_event(invalid[i], 0u, 0u,
                         NETCHESSZX_SESSION_EVENT_UNKNOWN, invalid[i]);
        }
        expect_event("ACK 1 d3", 0u, 0u,
                     NETCHESSZX_SESSION_EVENT_ACK_MOVE, "numeric ack");
        expect_event("NACK 65535 BUSY", 0u, 0u,
                     NETCHESSZX_SESSION_EVENT_NACK_MOVE, "numeric nack");
    }
    expect_event("BYE",
                 0u,
                 0u,
    ...
    expect_event("MOVE 1 d3",
                 0u,
                 0u,
                 NETCHESSZX_SESSION_EVENT_MOVE,
                 "move");
    expect_event("MOVE 1 d3 x4 junk",
                 0u,
                 0u,
                 NETCHESSZX_SESSION_EVENT_UNKNOWN,
    ...
    expect_event("CHAT ",
                 ...
                 0u,
                 NETCHESSZX_SESSION_EVENT_UNKNOWN,
                 "chat rejects empty text");
    expect_event("DRAW",
                 0u,
                 0u,
                 NETCHESSZX_SESSION_EVENT_UNKNOWN,
                 "removed draw ignored");
    expect_event("RESIGN",
                 0u,
                 0u,
...
static void test_mqtt_session_events(void)
{
    ...
    expect_event("DRAW",
                 ...
                 0u,
                 NETCHESSZX_SESSION_EVENT_UNKNOWN,
                 "mqtt removed draw ignored");
    expect_event("CANCEL DRAW",
                 1u,
                 0u,
                 NETCHESSZX_SESSION_EVENT_UNKNOWN,
                 "mqtt removed draw cancel ignored");
...
static void test_retained_policy(void)
{
    ...
    check(netchesszx_session_classify_event("legacy text", 1u, 1u, 0u) ==
              NETCHESSZX_SESSION_EVENT_UNKNOWN,
          "retained untyped text has no event");
    check(netchesszx_session_event_ignores_retained(
              NETCHESSZX_SESSION_EVENT_MACH_PC,
              1u),
          "retained machine identity ignored");
    check(!netchesszx_session_event_ignores_retained(
              NETCHESSZX_SESSION_EVENT_MQTT_HOST,
              1u),
...
static void test_mqtt_host_flags(void)
{
    ...
    flags = netchesszx_session_mqtt_host_flags("H W 10", 0u, 0u, &bad);
    check(!bad, "replacement live host valid");
    check(flags & NETCHESSZX_SESSION_MQTT_HOST_SESSION_CHANGED,
          "replacement session flagged even with unchanged color");
    check((flags & NETCHESSZX_SESSION_MQTT_HOST_COLOR_CHANGED) == 0u,
          "same-color replacement is not a color change");
