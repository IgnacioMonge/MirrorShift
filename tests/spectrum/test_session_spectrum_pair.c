...
static void test_direct_spectrum_pair_contract(void)
{
    ...
    check(netchesszx_session_has_local_turn(0u),
          "direct black host owns opening turn");
    ...
    check(!netchesszx_session_has_local_turn(0u),
          "direct white guest waits on opening turn");
}
...
static void test_mqtt_spectrum_pair_contract(void)
{
    ...
    rc = netchesszx_session_mqtt_apply_host_color(host_payload,
                                                  ...
                                                  &color_changed,
                                                  &new_live_session);
    check(rc == 2u, "mqtt Spectrum join accepts and activates host setup");
    check(netchesszx_mqtt_session_id == 4242u, "mqtt join stores session");
    check(netchesszx_host_color_ready == 1u,
          "mqtt join records authoritative host color");
    check(netchesszx_local_color == NETCHESSZX_COLOR_WHITE,
          "mqtt join becomes opposite host color");
    check(!netchesszx_session_has_local_turn(0u),
          "mqtt white guest waits on opening turn");
    ...
    netchesszx_mqtt_session_id = 4242u;
    check(netchesszx_session_has_local_turn(0u),
          "mqtt black host owns opening turn");
