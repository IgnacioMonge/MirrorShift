...
bool testDirectFraming()
{
    ...
    const QByteArray framed = QByteArrayLiteral("HELLO\r\nWORLD\n");
    const QVector<QByteArray> framedExpected = {"HELLO", "WORLD"};
    bool everyDirectFragment = true;
    for (qsizetype split = 0; split <= framed.size(); ++split) {
        DesktopTransportCodec fragmentCodec;
        QVector<QByteArray> fragmentLines;
        const auto firstPart = fragmentCodec.feedDirect(
            10u, framed.left(split),
            [&fragmentLines](const QByteArray &line) { fragmentLines.append(line); });
        const auto secondPart = fragmentCodec.feedDirect(
            10u, framed.mid(split),
            [&fragmentLines](const QByteArray &line) { fragmentLines.append(line); });
        everyDirectFragment = everyDirectFragment && !firstPart.overflow &&
                              !secondPart.overflow &&
                              fragmentLines == framedExpected;
    }
    ok = check(everyDirectFragment, "every direct fragment boundary preserves CRLF") && ok;
    return ok;
}
...
bool testMqttFramingAndDecode()
{
    ...
    const QByteArray encoded = DesktopTransportCodec::encodeMqttPublish(
        42u, QByteArray("netchesszx/v1/ROOM/w2b"), QByteArray("M 1 d3"), true);
    ...
    bool everyMqttFragment = true;
    for (qsizetype split = 0; split <= encoded.size(); ++split) {
        DesktopTransportCodec fragmentCodec;
        bool fragmentMalformed = false;
        const QVector<QByteArray> firstPart =
            fragmentCodec.feedMqtt(encoded.left(split), &fragmentMalformed);
        everyMqttFragment = everyMqttFragment && !fragmentMalformed;
        const QVector<QByteArray> secondPart =
            fragmentCodec.feedMqtt(encoded.mid(split), &fragmentMalformed);
        everyMqttFragment = everyMqttFragment && !fragmentMalformed &&
                            firstPart + secondPart == QVector<QByteArray>({encoded});
    }
    ok = check(everyMqttFragment, "every MQTT fragment boundary preserves packet") && ok;
    ...
    ok = check(packet.topic == "netchesszx/v1/ROOM/w2b" &&
               packet.payload == "M 1 d3", "publish contents") && ok;
