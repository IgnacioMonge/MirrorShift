...
static const MqttOwnedAction *findEndedAction(const MqttActionBatch &batch)
{
    for (uint8_t i = 0u; i < batch.count; ++i) {
        const MqttOwnedAction &action = batch.actions[i];
...
static void testReadyStartedEndedAndRoutes()
{
    ...
    adapter.enqueueLocalRequest(SESSION_REQUEST_MOVE, 0u, "d3",
...
{
    MqttSessionAdapter remote;
    MqttSessionAdapter local;
    MqttSessionAdapter lost;
    MqttActionBatch batch;
    const MqttOwnedAction *ended;
    uint8_t byeTx;
    uint8_t offlineTx;

    reachReady(remote, batch);
    check(remote.enqueueRx(1u, "w2b", false, "BYE") &&
              remote.takeNextBatch(&batch),
          "remote MQTT BYE drains");
    ended = findEndedAction(batch);
    check(ended != nullptr &&
              ended->action.data.session.reason ==
    ...
    check(local.takeNextBatch(&batch),
          "local MQTT BYE offline cleanup drains");
    ...
    ended = findEndedAction(batch);
    check(ended != nullptr &&
              ended->action.data.session.reason == SESSION_END_REASON_LOCAL_BYE,
    ...
    MqttSessionAdapter guestTimeout;
    reachReady(guestTimeout, batch);
    guestTimeout.enqueueTimeout(SESSION_TIMER_LIVENESS);
    check(guestTimeout.takeNextBatch(&batch), "guest liveness ping drains");
    byeTx = sendId(batch);
    check(byeTx != 0u, "guest liveness ping sends");
    guestTimeout.enqueueTxResult(byeTx, SESSION_TX_OK);
    check(guestTimeout.takeNextBatch(&batch), "guest ping ok drains");
    guestTimeout.enqueueTimeout(SESSION_TIMER_LIVENESS);
    check(guestTimeout.takeNextBatch(&batch),
          "guest peer deadline timeout drains");
    ended = findEndedAction(batch);
    check(ended != nullptr &&
              ended->action.data.session.reason ==
                  SESSION_END_REASON_REMOTE_BYE,
          "guest liveness timeout marks remote bye");
}
