...
static uint8_t host_overlay_context[NETCHESSZX_LOWRAM_OVERLAY_CONTEXT_SIZE];
#undef NETCHESSZX_LOWRAM_OVERLAY_CONTEXT_ADDR
#define NETCHESSZX_LOWRAM_OVERLAY_CONTEXT_ADDR \
    ((uintptr_t)host_overlay_context)
...
static uint8_t host_takeback_prompt_race;
...
static uint16_t host_last_notify_msg;
...
static uint8_t host_hidden_piece_count;
...
static uint8_t host_turn_label_mode;
static uint8_t host_game_timers[6];
static uint8_t host_game_timer_start_calls;
static uint8_t host_game_timer_stop_calls;
static uint8_t host_game_timer_restore_calls;
static uint8_t host_saved_file_payload[NETCHESSZX_SAVE_WIRE_B64_SIZE];
static uint8_t host_saved_file_valid;
static uint8_t host_move_history_calls;
static uint8_t host_move_history_author;
static uint8_t host_hint_clear_calls;
static uint8_t host_hint_show_calls;
static const uint8_t *host_restore_file_payload;
static uint8_t host_restore_apply_pending;
static uint8_t host_restore_move_entries;
...
int16_t spxn_poll(void) { return 0; }
int16_t spxn_recv(void *buffer, uint16_t maximum)
{
    (void)buffer;
    (void)maximum;
...
int16_t spxn_accept(void) { return SPXN_OK; }
int16_t spxn_listen(uint16_t port)
{
    (void)port;
    return SPXN_OK;
...
static uint8_t direct_feed_uart_byte_ovl(uint8_t c)
{
    ...
    if (c == '\n') {
        host_spxn_header[host_spxn_header_len] = '\0';
        if (host_spxn_header_len >= 8u && host_spxn_header[1] == ',' &&
            strncmp(host_spxn_header + 2u, "CLOSED", 6u) == 0) {
            direct_link_closed = (uint8_t)(
                host_spxn_header[0] == (char)('0' + active_link));
        }
        host_spxn_header_len = 0u;
        return 0u;
    }
...
uint8_t spectrum_net_runtime_clock_ready(void) { return 0u; }
...
static void check_restore_codec_contract(void)
{
    static const char expected_b64[] =
        "AAAAAAAAAAAAACAAAAEgAAABEAAAAQAAAAAAAAAAAABAAAAAAQEAAAAAOwIZ";
    static const char expected_chunk_0[] =
        "AAAAAAAAAAAAACAAAAEgAAABEAAAAQ";
    static const char expected_chunk_1[] =
        "AAAAAAAAAAAABAAAAAAQEAAAAAOwIZ";
    static const char mirrorshift_position[65] =
        "........"
        "........"
        "....B..."
        "...AB..."
        ...
        "........"
        "........";
    spectrum_reversi_snapshot_t source;
    spectrum_reversi_snapshot_t decoded;
    netchesszx_save_meta_t source_meta = { 0 };
    netchesszx_save_meta_t decoded_meta = { 0 };
    char b64[NETCHESSZX_SAVE_WIRE_B64_SIZE];

    memcpy(source.cells, mirrorshift_position, sizeof(source.cells));
    source.side = 0u;
    source.over = 0u;
...
static void check_restore_snapshot_keeps_active_seating(void)
{
    ...
    spectrum_reversi_snapshot_save(&snapshot);
    host_color = netchesszx_host_color;
    local_color = netchesszx_local_color;
    meta.host_color = NETCHESSZX_SAVE_HOST_BLACK;
    restore_rx_mask = 0u;
    check(saveload_apply_snapshot(&snapshot, &meta),
          "valid restore snapshot applies");
    check(netchesszx_host_color == host_color &&
              netchesszx_local_color == local_color,
          "restore snapshot cannot replace active seating");
}

static void check_save_restore_preserves_live_timers(void)
{
    static const uint8_t active_timers[6] = { 1u, 2u, 3u, 4u, 5u, 6u };
    static const uint8_t ended_timers[6] = { 7u, 8u, 9u, 10u, 11u, 12u };
    spectrum_reversi_snapshot_t snapshot;
    spectrum_reversi_snapshot_t ended;
    spectrum_reversi_snapshot_t decoded;
    netchesszx_save_meta_t meta = { 0 };
    ...
    game_over = 0u;
    memcpy(host_game_timers, active_timers, sizeof(active_timers));
    host_saved_file_valid = 0u;
    local_save_game("TIMER");
    check(host_saved_file_valid &&
              spectrum_restore_decode((const char *)host_saved_file_payload,
                                      &decoded, &decoded_meta) &&
              memcmp(decoded_meta.timers, active_timers,
                     sizeof(active_timers)) == 0,
          "local save serializes nonzero live timers");

    memset(host_game_timers, 0, sizeof(host_game_timers));
    host_game_timer_start_calls = 0u;
    host_game_timer_stop_calls = 0u;
    host_game_timer_restore_calls = 0u;
    host_restore_control_pending = 0u;
    host_restore_apply_pending = 0u;
    host_started_seen = 1u;
    meta.ply = 9u;
    meta.flags = NETCHESSZX_SAVE_FLAG_ACTIVE;
    memcpy(meta.timers, active_timers, sizeof(active_timers));
    check(saveload_apply_snapshot(&snapshot, &meta) &&
              host_game_timer_start_calls == 1u &&
              host_game_timer_stop_calls == 0u &&
              host_game_timer_restore_calls == 1u &&
              memcmp(host_game_timers, active_timers,
                     sizeof(active_timers)) == 0,
          "active restore resumes with saved timers");

    memset(ended.cells, MS_CELL_A, sizeof(ended.cells));
    ended.side = MS_SIDE_A;
    ended.over = 1u;
    host_game_timer_start_calls = 0u;
    host_game_timer_stop_calls = 0u;
    host_game_timer_restore_calls = 0u;
    meta.flags = NETCHESSZX_SAVE_FLAG_ACTIVE |
                 NETCHESSZX_SAVE_FLAG_GAME_OVER;
    memcpy(meta.timers, ended_timers, sizeof(ended_timers));
    check(saveload_apply_snapshot(&ended, &meta) &&
              host_game_timer_start_calls == 0u &&
              host_game_timer_stop_calls == 1u &&
              host_game_timer_restore_calls == 1u &&
              memcmp(host_game_timers, ended_timers,
                     sizeof(ended_timers)) == 0 &&
              game_over && !local_turn,
          "ended restore keeps saved timers stopped");
...
read_next_step:
    ...
    if (host_takeback_prompt_race == 1u &&
        step->length == 9u && memcmp(step->payload, "MOVE 2 c3", 9u) == 0) {
        memcpy(local_input, "/takeback", 10u);
        local_input_len = local_input_cursor = 9u;
        local_input_mode = 1u;
        check(process_local_key(13u) &&
                  confirm_action == CONFIRM_TAKEBACK_SEND,
              "open unsent takeback confirmation before opponent move");
        host_takeback_prompt_race = 2u;
    } else if (host_takeback_prompt_race == 2u) {
        check(confirm_action == CONFIRM_NONE && game_ply == 2u &&
                  takeback_pending_ply == 0u,
              "valid opponent move cancels unsent takeback");
        host_takeback_prompt_race = 0u;
    }
...
uint8_t spectrum_net_send_text(const char *text)
{
    ...
    /* MACH is advisory and must not alter the session parity transcript. */
    if (strncmp(text, NETCHESS_PROTO_MACH_PREFIX,
                sizeof(NETCHESS_PROTO_MACH_PREFIX) - 1u) == 0) {
        return 1u;
    }
...
uint8_t spectrum_gui_poll_key(void)
{
    ...
    if (host_local_confirm_pending) {
        ...
        if (confirm_action == CONFIRM_RESET_SEND) {
            host_local_reset_pending = 1u;
        }
...
static void check_move_parser_contract(void)
{
    ...
    /* Golden branch map for entry_input_edit.asm's move parser:
       Leading-space loop: "  d3" / tab reject.
...
static const char *host_status_error;
void spectrum_gui_set_status_error(const char *text) { host_status_error = text; }
void spectrum_gui_game_timer_start(void)
{
    ++host_game_timer_start_calls;
...
{
    memcpy(timers, host_game_timers, sizeof(host_game_timers));
}
void spectrum_gui_game_timer_restore(const uint8_t *timers)
{
    ++host_game_timer_restore_calls;
    memcpy(host_game_timers, timers, sizeof(host_game_timers));
}
void spectrum_gui_move_timer_reset(void) {}
void spectrum_gui_set_turn_label(uint8_t mode) { host_turn_label_mode = mode; }
...
void spectrum_gui_notify_msg(uint16_t packed)
{
    ...
    host_last_notify_msg = packed;

    switch (msg_id) {
    case SPECTRUM_GUI_MSG_LOAD_CANCELLED:
        spectrum_gui_notify(NETCHESSZX_UI_NOTICE_LOAD_CANCELLED, 0u);
        break;
    case SPECTRUM_GUI_MSG_CONTROL_RESET_CANCELLED:
        spectrum_gui_notify(NETCHESSZX_UI_NOTICE_CONTROL_RESET_CANCELLED, 0u);
        break;
    case SPECTRUM_GUI_MSG_CONTROL_RESET_EXPIRED:
        spectrum_gui_notify(NETCHESSZX_UI_NOTICE_CONTROL_RESET_EXPIRED, 0u);
        break;
    default:
        break;
    }
...
uint8_t spectrum_net_runtime_shift_clock(int8_t hour_delta) { (void)hour_delta; return 1u; }
...
uint8_t spectrum_gui_board_pieces_visible(void) { return host_pieces_visible; }
...
void spectrum_gui_hide_board_pieces(void)
{
    ...
    host_hidden_piece_count = 0u;
    for (i = 0u; i < MS_CELLS; ++i) {
        if (cells[i] != MS_CELL_EMPTY) {
            ++host_hidden_piece_count;
        }
    }
    host_pieces_visible = 0u;
}
...
void spectrum_gui_add_move(const char *ply, const char *move)
{
    (void)ply;
    if (move != 0) {
        ++host_move_history_calls;
        host_move_history_author =
            spectrum_overlay_context[SPECTRUM_OVL_CTX_GUI_MOVE_AUTHOR];
    }
    if (host_restore_apply_pending) {
        ++host_restore_move_entries;
    }
}
...
void spectrum_board_show_legal_hints(uint8_t from_row, uint8_t from_col)
{
    (void)from_row;
    (void)from_col;
    ++host_hint_show_calls;
    netchesszx_hinted_rows[0] = 1u;
}
...
{
    (void)name;
    if (entry == SPECTRUM_OVL_SAVELOAD_SAVE_NCZS && buf != 0) {
        memcpy(host_saved_file_payload, buf,
               sizeof(host_saved_file_payload));
        host_saved_file_valid = 1u;
        return 1u;
    }
                            ...
                            DirectParityTrace *trace)
{
    ...
    /* Production enters the link loop with the setup seed preview visible. */
    ...
    if (host_failed || host_timeout_active || host_fail_length != 0u ||
        ...
        host_control_reset_pending || host_local_confirm_pending ||
        ...
        fprintf(stderr,
                ...
                "reset=%u confirm=%u local_reset=%u "
...
static void check_mqtt_peer_loss_blocks_taboption(void)
{
    ...
    check(host_menu_key_calls == 2u,
          "TABOPTION handler reached after new peer");
    check(host_menu_visible,
          "TABOPTION available again after new peer");
    spectrum_gui_hide_menu();
    game_over = 1u;
    check(process_local_key(SPECTRUM_GUI_KEY_MENU),
          "TABOPTION key consumed at rematch boundary");
    check(host_menu_key_calls == 3u,
          "TABOPTION handler reached at rematch boundary");
    check(host_menu_visible,
...
static void check_mqtt_peer_identity_reset(void)
{
    ...
    status_phase_current =
        SPECTRUM_STATUS_PACK(STATUS_PHASE_CONNECTED, NETCHESS_PLAT_MAC);
    host_status_phase_calls = 0u;

    check(session_presence_handle_event(
              NETCHESSZX_SESSION_EVENT_MQTT_PEER_OFFLINE,
              payload, 0u) == SESSION_DISPATCH_HANDLED,
          "MQTT peer-offline resets peer identity");
    check(host_status_phase_calls != 0u &&
          ...
          "MQTT peer-offline clears peer platform");

    check(session_presence_handle_event(NETCHESSZX_SESSION_EVENT_MACH_PC,
                                        payload, 0u) ==
              SESSION_DISPATCH_HANDLED &&
              SPECTRUM_STATUS_UNPACK_PHASE(host_status_phase) ==
                  STATUS_PHASE_CONNECTED &&
              SPECTRUM_STATUS_UNPACK_PLATFORM(host_status_phase) ==
                  NETCHESS_PLAT_PC,
          "replacement peer platform updates status only");
}

static void check_incoming_takeback_waits_for_decision(void)
{
    DirectParityTrace trace = {0};
    char request[] = "TAKEBACK 2";
    char nack[] = "NACK 2";
    uint8_t transport;

    for (transport = NETCHESSZX_TRANSPORT_DIRECT;
         transport <= NETCHESSZX_TRANSPORT_MQTT; ++transport) {
        netchesszx_session_configure(NETCHESSZX_SESSION_ROLE_HOST,
                                     transport, NETCHESSZX_COLOR_BLACK);
        host_trace = &trace;
        host_link_id = 1u;
        host_failed = 0u;
        host_fail_payload = 0;
        ...
        game_over = 0u;
        confirm_action = CONFIRM_NONE;
        control_pending = start_pending = resign_pending = restore_rx_mask = 0u;
        pending_local_clear();
        takeback_clear();
        game_ply = takeback_snapshot_ply = 2u;
        trace.count = 0u;
        session_control_handle_event(NETCHESSZX_SESSION_EVENT_TAKEBACK, request);
        check(confirm_action == CONFIRM_TAKEBACK_ACCEPT && takeback_pending_ply == 2u,
              "incoming TAKEBACK opens correlated confirmation");
        trace.count = 0u;
        check(!local_retry_pending(), "incoming TAKEBACK never starts outgoing retry timeout");
        check(retry_pending_outgoing() && trace.count == 0u,
              "incoming TAKEBACK is not echoed to its requester");
        session_control_handle_event(NETCHESSZX_SESSION_EVENT_NACK_MOVE, nack);
        check(confirm_action == CONFIRM_TAKEBACK_ACCEPT && takeback_pending_ply == 2u,
              "unrelated NACK cannot erase incoming TAKEBACK correlation");
        trace.count = 0u;
        process_local_key(13u);
        process_local_key(' ');
        check(confirm_action == CONFIRM_TAKEBACK_ACCEPT && trace.count == 0u,
              "board selection keys do not answer TAKEBACK");
        process_local_key('n');
        check(confirm_action == CONFIRM_NONE && takeback_pending_ply == 0u &&
                  trace.count == 1u && strcmp(trace.observations[0].payload, "NACK 2") == 0,
              "explicit rejection replies with original ply");
        trace.count = 0u;
        session_control_handle_event(NETCHESSZX_SESSION_EVENT_TAKEBACK, request);
        check(confirm_action == CONFIRM_TAKEBACK_ACCEPT && takeback_pending_ply == 2u,
              "same-ply TAKEBACK prompts again after rejection");
        confirm_action = CONFIRM_NONE;
        takeback_snapshot_local = 1u;
        trace.count = 0u;
        check(local_retry_pending() && retry_pending_outgoing() && trace.count == 1u &&
                  strcmp(trace.observations[0].payload, "TAKEBACK 2") == 0,
              "outgoing TAKEBACK retains retry behavior");
        session_control_handle_event(NETCHESSZX_SESSION_EVENT_NACK_MOVE, nack);
        check(takeback_pending_ply == 0u, "outgoing TAKEBACK still consumes correlated rejection");
    }
    host_trace = 0;
    game_status_active = 0u;
    takeback_clear();
...
static void check_reset_ack_replay_window(void)
{
    DirectParityTrace trace = {0};
    char payload[] = "RESET";
    ...
    reset_ack_replay_enabled = 1u;
    reset_ack_replay_polls = 1u;

    check(session_control_handle_event(NETCHESSZX_SESSION_EVENT_RESET,
                                       payload) == SESSION_DISPATCH_HANDLED,
          "RESET retry handled inside ACK replay window");
    check(trace.count == 1u &&
              trace.observations[0].type == DIRECT_PARITY_OBS_SEND &&
              strcmp(trace.observations[0].payload, "ACK RESET") == 0,
          "RESET retry is re-ACKed without a second prompt");
    ...
    check(session_control_handle_event(NETCHESSZX_SESSION_EVENT_RESET,
              ...
              trace.count == 1u &&
              trace.observations[0].type == DIRECT_PARITY_OBS_DECISION &&
              trace.observations[0].code == DIRECT_PARITY_REQUEST_RESET,
          "fresh RESET prompts after replay window expires");

    confirm_action = CONFIRM_NONE;
    reset_ack_replay_enabled = 0u;
    reset_ack_replay_polls = 0u;
}
...
static void check_direct_disconnect_reason_messages(void)
{
    char bye[] = "BYE";
    ...
    netchesszx_session_peer_mark_ready();
    host_status_error = 0;
    check(session_presence_handle_event(NETCHESSZX_SESSION_EVENT_BYE,
                                        bye, 0u) == SESSION_DISPATCH_EXIT &&
              host_status_error != 0 &&
              strcmp(host_status_error,
                     NETCHESSZX_UI_ERROR_OPPONENT_DISCONNECTED) == 0,
          "DIRECT BYE reports intentional opponent disconnect");

    host_status_error = 0;
    handle_opponent_disconnected();
    check(host_status_error != 0 &&
              strcmp(host_status_error,
                     NETCHESSZX_UI_ERROR_CONNECTION_LOST) == 0,
          "DIRECT link loss reports connection lost");
}

static void check_cursor_sends_placement(void)
{
    DirectParityTrace trace = { 0 };
    ...
    check(local_turn && cursor_row == 2u && cursor_col == 4u &&
              spectrum_board_is_legal_move("e6"),
          "black host starts on the first legal opening square");
    trace.count = 0u;
    host_failed = 0u;

    check(cursor_select_or_move('\r'), "one-click placement consumed");
    check(!host_failed && trace.count == 1u &&
              trace.observations[0].type == DIRECT_PARITY_OBS_SEND &&
              strcmp(trace.observations[0].payload, "MOVE 1 e6") == 0 &&
              pending_local_ply == 1u &&
              strcmp(pending_local_move, "e6") == 0,
          "one-click placement sends two-byte MOVE");
...
static void check_reset_hides_old_position_before_rules_reset(void)
{
    const char *cells;
    uint8_t i;
    ...
    netchesszx_session_peer_mark_ready();
    spectrum_board_reset();
    check(spectrum_board_apply_trusted_move("d3"),
          "reset regression position accepts one opening move");
    game_status_active = 1u;
    game_over = 0u;
    host_pieces_visible = 1u;
    host_hidden_piece_count = 0u;
    ...
    check(host_hidden_piece_count == 5u,
          "reset hides the old visible position before rules reset");
    check(piece_count == 4u && host_pieces_visible,
          "reset shows exactly the four initial pieces");
...
static void check_hint_turn_lifecycle(void)
{
    uint8_t saved_hints = netchesszx_movement_hints;
    ...
    memset(netchesszx_hinted_rows, 0, sizeof(netchesszx_hinted_rows));
    host_hint_clear_calls = 0u;
    host_hint_show_calls = 0u;

    turn_set_notice(1u, 0u);
    check(host_hint_show_calls == 1u && netchesszx_hinted_rows[0] != 0u,
          "local turn computes legal hints");

    turn_set_notice(0u, 0u);
    check(host_hint_clear_calls == 1u && netchesszx_hinted_rows[0] == 0u,
          "opponent turn clears legal hints");

    netchesszx_hinted_rows[0] = 1u;
    turn_set_notice(1u, 0u);
    check(host_hint_clear_calls == 2u && host_hint_show_calls == 2u &&
              netchesszx_hinted_rows[0] != 0u,
          "new local turn replaces stale hints");
...
static void check_forced_silence_turn_label(void)
{
    ...
    memcpy(state.cells, position, sizeof(state.cells));
    state.side = MS_SIDE_A;
    state.over = 0u;
    check(ms_rules_restore(&state) == MS_OK,
          "forced-Silence position restored");
    check(spectrum_board_apply_trusted_move("c1") &&
              spectrum_board_last_silences() == 1u &&
              spectrum_board_side() == MS_SIDE_A,
          "forced Silence returns the turn to A");
    ...
    netchesszx_session_configure(NETCHESSZX_SESSION_ROLE_HOST,
                                 ...
                                 NETCHESSZX_COLOR_WHITE);
    status_refresh_game();
    check(host_turn_label_mode ==
              (SPECTRUM_GUI_TURN_SELF_SILENCE |
               SPECTRUM_GUI_TURN_MARKER_WHITE),
          "local forced Silence uses SELF SILENCE label");
    ...
    check(host_turn_label_mode ==
              (SPECTRUM_GUI_TURN_ECHO_SILENCE |
               SPECTRUM_GUI_TURN_MARKER_WHITE),
          "remote forced Silence uses ECHO SILENCE label");
...
static void check_local_move_ply_overflow(void)
{
    ...
    check(send_local_move("d6") == LOCAL_MOVE_REJECTED &&
              host_uart_tx_len == 0u &&
              pending_local_ply == 0u &&
              game_ply == 65535u,
...
static void check_chat_editor_during_pending_control(void)
{
    ...
    check(process_local_key(13u), "chat editor opens during pending reset");
    check(local_input_mode, "pending reset does not block chat editor");
    check(process_local_key(SPECTRUM_GUI_KEY_MENU),
...
static void check_forced_silence_history_author(void)
{
    static const char *moves[] = {
        "d3", "c3", "b3", "b2", "b1", "a1", "c4", "c1", "c2",
        "d2", "d1", "e1", "a2", "a3", "f5", "e2", "f1", "g1"
    };
    uint8_t i;

    spectrum_board_reset();
    for (i = 0u; i < sizeof(moves) / sizeof(moves[0]); ++i) {
        check(spectrum_board_apply_trusted_move(moves[i]),
              "history reaches Silence through legal opening moves");
    }
    check(spectrum_board_last_silences() == 1u &&
              spectrum_board_side() == MS_SIDE_A,
          "ply eighteen Silence leaves white to move again");
    netchesszx_session_configure(NETCHESSZX_SESSION_ROLE_HOST,
                                 NETCHESSZX_TRANSPORT_DIRECT,
                                 NETCHESSZX_COLOR_WHITE);
    netchesszx_session_peer_mark_ready();
    ...
    game_over = 0u;
    game_ply = 18u;
    pending_local_ply = 19u;
    memcpy(pending_local_move, "f2", 3u);
    host_move_history_calls = 0u;
    host_move_history_author = 0u;

    apply_pending_local_move(0);
    check(host_move_history_calls == 1u &&
              game_ply == 19u &&
              host_move_history_author ==
                  SPECTRUM_OVL_GUI_MOVE_AUTHOR_VALID,
          "forced Silence records the pre-move white author at odd ply");
    game_status_active = 0u;
    game_over = 0u;
    pending_local_clear();
    spectrum_board_reset();
...
static void check_restore_receive_blocks_local_input(void)
{
    netchesszx_session_configure(NETCHESSZX_SESSION_ROLE_HOST,
                                 NETCHESSZX_TRANSPORT_DIRECT,
                                 NETCHESSZX_COLOR_WHITE);
    ...
    check(process_local_key(13u) == 1u && !local_input_mode,
          "restore receive blocks editor");
    check(process_local_key(SPECTRUM_GUI_KEY_MENU_FILE) == 1u &&
              !host_failed && !host_fileui_visible,
          "restore receive blocks FILE");
    check(process_local_key(32u) == 1u && host_uart_tx_len == 0u,
          "restore receive blocks board move");

    {
        DirectParityTrace trace = { 0 };

        memcpy(local_input, "hello", 6u);
        local_input_len = 5u;
        local_input_cursor = 5u;
        local_input_mode = 1u;
        host_trace = &trace;
        host_last_notify_msg = 0xffffu;
        check(input_submit() == 1u && trace.count == 0u &&
                  local_input_mode &&
                  SPECTRUM_GUI_MSG_ID(host_last_notify_msg) ==
                      SPECTRUM_GUI_MSG_LOAD_WAITING_APPROVAL &&
                  SPECTRUM_GUI_MSG_KIND(host_last_notify_msg) ==
                      SPECTRUM_GUI_MSG_KIND_WAIT,
              "restore pending blocks CHAT with load-wait notice");

        restore_rx_mask = 0u;
        check(input_submit() == LOCAL_KEY_CHAT_SENT &&
                  trace.count == 2u &&
                  strcmp(trace.observations[0].payload, "CHAT hello") == 0 &&
                  trace.observations[1].type == DIRECT_PARITY_OBS_CHAT &&
                  strcmp(trace.observations[1].payload, "hello") == 0 &&
                  !local_input_mode,
              "CHAT sends unchanged after restore completes");
        host_trace = 0;
    }

    restore_rx_mask = RESTORE_RX_APPLIED;
    check(process_local_key(13u) == 1u && local_input_mode,
          "restore applied does not freeze input");
    edit_stop_clear();
    restore_rx_mask = 0u;
    game_status_active = 0u;
    netchesszx_session_peer_clear();
...
static void check_pending_move_blocks_chat(void)
{
    DirectParityTrace trace = { 0 };
    ...
    control_pending = 0u;
    restore_rx_mask = 0u;
    pending_local_ply = 7u;
    memcpy(pending_local_move, "d3", 3u);
    memcpy(local_input, "hello", 6u);
    local_input_len = 5u;
    local_input_cursor = 5u;
    local_input_mode = 1u;
    ...
    check(input_submit() == 1u && trace.count == 0u &&
              local_input_mode && strcmp(local_input, "hello") == 0 &&
              SPECTRUM_GUI_MSG_ID(host_last_notify_msg) ==
                  SPECTRUM_GUI_MSG_RESET_WAIT,
          "pending MOVE blocks CHAT without losing editor text");
    pending_local_clear();
    check(input_submit() == LOCAL_KEY_CHAT_SENT && trace.count == 2u &&
              strcmp(trace.observations[0].payload, "CHAT hello") == 0 &&
              trace.observations[1].type == DIRECT_PARITY_OBS_CHAT &&
              strcmp(trace.observations[1].payload, "hello") == 0 &&
              !local_input_mode,
          "CHAT resumes unchanged after pending MOVE clears");
...
#ifdef NETCHESSZX_NEXT
static void check_direct_rearms_hard_reset_budget(void)
{
    last_ip[NETCHESSZX_LOWRAM_LAST_IP_SIZE - 1u] = 0u;
    check(direct_prepare_link_ovl() &&
              last_ip[NETCHESSZX_LOWRAM_LAST_IP_SIZE - 1u] == 1u,
          "first Direct attempt rearms one hard reset");
    last_ip[NETCHESSZX_LOWRAM_LAST_IP_SIZE - 1u] = 0u;
    check(direct_prepare_link_ovl() &&
              last_ip[NETCHESSZX_LOWRAM_LAST_IP_SIZE - 1u] == 1u,
          "new Direct attempt rearms one hard reset");
}
#endif
...
int main(int argc, char **argv)
{
    ...
    check_restore_snapshot_keeps_active_seating();
    check_save_restore_preserves_live_timers();
    check_move_parser_contract();
    check_text_helper_contract();
    check_input_editor_driver();
    check_cursor_sends_placement();
    check_reset_hides_old_position_before_rules_reset();
    check_hint_turn_lifecycle();
    check_forced_silence_turn_label();
    check_forced_silence_history_author();
    check_local_move_ply_overflow();
    check_chat_editor_during_pending_control();
    check_non_chat_input_liveness_status();
    check_restore_receive_blocks_local_input();
    check_pending_move_blocks_chat();
#ifdef NETCHESSZX_NEXT
    check_direct_rearms_hard_reset_budget();
#endif
    check_incoming_takeback_waits_for_decision();
    check_reset_ack_replay_window();
    check_mqtt_peer_loss_blocks_taboption();
    check_mqtt_peer_identity_reset();
    check_direct_guest_disconnect_forgets_side();
    check_direct_disconnect_reason_messages();
    for (i = 0u; i < direct_parity_scenario_count; ++i) {
        ...
        for (variant = 0u; variant < 2u; ++variant) {
            if (variant != 0u &&
                strcmp(scenario->id, "takeback-latch-next-move") != 0) {
                break;
            }
            host_takeback_prompt_race = variant;
            ...
            check(host_takeback_prompt_race == 0u, "unsent takeback race exercised");
        }
    }
