...
static void test_moves_and_tx_guard(void)
{
    ...
    local_request(&local, SESSION_REQUEST_MOVE, 0u, "d3");
    expect_send(&local, "MOVE 1 d3", 1u, "local move send");
...
static void test_retry_and_failure_paths(void)
{
    ...
    make_active(&f, SESSION_ROLE_HOST, SESSION_COLOR_WHITE);
    local_request(&f, SESSION_REQUEST_MOVE, 0u, "d3");
    tx_result(&f, SESSION_TX_OK);
    for (i = 0u; i < SESSION_DIRECT_REPLY_RETRIES; ++i) {
        timeout(&f, SESSION_TIMER_CONTROL);
        expect_send(&f, "MOVE 1 d3", 1u, "move retry");
...
static void test_restore(void)
{
    ...
    check(local_request(&host, SESSION_REQUEST_MOVE, 0u, "d3") == 0u,
...
static void test_duplicate_hello_and_reconnect(void)
{
    ...
    check(f.actions[1].data.session.reason ==
              SESSION_END_REASON_TRANSPORT_LOST,
          "link down marks transport loss");
...
static void test_lost_ack_and_move_conflict(void)
{
    ...
    check(local_action->data.game.kind == SESSION_DELIVER_LOCAL_MOVE &&
              local_action->data.game.length == SESSION_MOVE_TEXT_LEN &&
              memcmp(local_action->data.game.payload, "d3", 2u) == 0,
    ...
    check(remote_action->data.game.kind == SESSION_DELIVER_REMOTE_MOVE &&
              ...
              remote_action->data.game.length == SESSION_MOVE_TEXT_LEN &&
              memcmp(remote_action->data.game.payload, "c4", 2u) == 0,
    ...
    check(f.state.pending_control == SESSION_REQUEST_MOVE &&
              ...
              memcmp(f.workspace.move, "d3", 2u) == 0,
...
static void test_invalid_local_preserves_state(void)
{
    ...
    check(local_request(&f, SESSION_REQUEST_MOVE, 0u, "e2e4") == 0u,
          "inherited four-byte local move rejected");
    check(local_request(&f, SESSION_REQUEST_MOVE, 0u, "e7e8q") == 0u,
          "inherited five-byte local move rejected");
    check(memcmp(&f.state, &before, sizeof(before)) == 0 &&
              memcmp(&f.workspace, &workspace_before,
                     sizeof(workspace_before)) == 0,
          "inherited local moves preserve state");
...
static void test_reset_cancel_timeout(void)
{
    Fixture f;
    uint8_t role;
    uint8_t retry;
    ...
    for (role = SESSION_ROLE_HOST; role <= SESSION_ROLE_GUEST; ++role) {
        make_active(&f, role,
                    role == SESSION_ROLE_HOST ? SESSION_COLOR_WHITE
                                              : SESSION_COLOR_BLACK);
        local_request(&f, SESSION_REQUEST_RESET, 0u, 0);
        expect_send(&f, "RESET", 1u, "control request sent");
        tx_result(&f, SESSION_TX_OK);
        for (retry = 0u; retry < SESSION_DIRECT_REPLY_RETRIES; ++retry) {
            timeout(&f, SESSION_TIMER_CONTROL);
            expect_send(&f, "RESET", 1u, "control transport retry sent");
            tx_result(&f, SESSION_TX_OK);
        }
        timeout(&f, SESSION_TIMER_CONTROL);
        check(find_action(&f, SESSION_ACT_LINK_CLOSE) == 0,
        ...
        timeout(&f, SESSION_TIMER_CONTROL);
        expect_send(&f, "CANCEL RESET", 1u, "control cancellation sent");
        tx_result(&f, SESSION_TX_OK);
        rx(&f, 1u, "NACK RESET");
...
static void test_short_tx_is_atomic(void)
{
    ...
    event.data.local.payload = (const uint8_t *)"d3";
...
static void test_unsupported_draw_and_crossed_reset(void)
{
    Fixture f;
    SessionState before;
    SessionWorkspace workspace_before;
    const char *frames[] = {"DRAW", "ACK DRAW", "NACK DRAW", "NACK DRAW BUSY", "CANCEL DRAW"};
    unsigned i;

    make_active(&f, SESSION_ROLE_HOST, SESSION_COLOR_WHITE);
    f.state.liveness_misses = 1u;
    before = f.state;
    workspace_before = f.workspace;
    for (i = 0u; i < sizeof(frames) / sizeof(frames[0]); ++i) {
        check(rx(&f, 1u, frames[i]) == 0u &&
                  memcmp(&f.state, &before, sizeof(before)) == 0 &&
                  memcmp(&f.workspace, &workspace_before, sizeof(workspace_before)) == 0,
              "removed DRAW frame is inert including liveness");
    }
    check(local_request(&f, 5u, 0u, 0) == 0u &&
              memcmp(&f.state, &before, sizeof(before)) == 0,
          "removed local request ID is inert");
...
static void test_game_results_and_takeback(void)
{
    ...
    expect_send(&f, "NACK 1 REJECT", 1u,
                "exact rejected placement replays rejection");
    tx_result(&f, SESSION_TX_OK);
    rx(&f, 1u, "MOVE 1 a1");
    check(find_action(&f, SESSION_ACT_DELIVER_GAME) != 0,
          "corrected placement at rejected ply is validated afresh");
...
static void test_link_zero_busy_bye_and_order(void)
{
    ...
    rx(&f, 1u, "BYE");
    action = find_action(&f, SESSION_ACT_SESSION_CHANGED);
    check(find_action(&f, SESSION_ACT_LINK_CLOSE) != 0 && action != 0 &&
              action->data.session.reason == SESSION_END_REASON_REMOTE_BYE,
    ...
    action = find_action(&f, SESSION_ACT_SESSION_CHANGED);
    check(find_action(&f, SESSION_ACT_LINK_CLOSE) != 0 && action != 0 &&
              action->data.session.reason == SESSION_END_REASON_LOCAL_BYE,
...
static void test_u16_wire_edges(void)
{
    ...
    local_request(&f, SESSION_REQUEST_MOVE, 0u, "a3");
    expect_send(&f, "MOVE 10000 a3", 1u, "five-digit move formatting");

    make_active(&f, SESSION_ROLE_HOST, SESSION_COLOR_WHITE);
    f.state.current_ply = 65534u;
    local_request(&f, SESSION_REQUEST_MOVE, 0u, "a3");
    expect_send(&f, "MOVE 65535 a3", 1u, "maximum u16 formatting");
}
...
static void test_chat_while_control_pending(void)
{
    ...
    tx_result(&f, SESSION_TX_OK);
    local_request(&f, SESSION_REQUEST_CHAT, 0u, "hello");
    expect_send(&f, "CHAT hello", 1u, "chat sends while reset is pending");
    check(f.state.pending_control == SESSION_REQUEST_RESET,
          "chat preserves pending reset");
    tx_result(&f, SESSION_TX_OK);
    action = find_action(&f, SESSION_ACT_DELIVER_GAME);
    check(action != 0 && action->data.game.kind == SESSION_DELIVER_CHAT &&
              action->data.game.value == SESSION_CHAT_LOCAL &&
              action->data.game.length == 5u &&
              memcmp(action->data.game.payload, "hello", 5u) == 0,
          "chat handoff remains local while reset is pending");
    check(f.state.pending_control == SESSION_REQUEST_RESET,
          "chat handoff preserves pending reset");
    ...
    check(local_request(&f, SESSION_REQUEST_RESIGN, 0u, 0) == 0u &&
              ...
              memcmp(&f.workspace, &workspace_before,
                     sizeof(workspace_before)) == 0,
          "second control stays blocked while reset is pending");
...
static void test_rx_liveness_requires_classified_payload(void)
{
    Fixture f;

    make_active(&f, SESSION_ROLE_HOST, SESSION_COLOR_WHITE);
    f.state.liveness_misses = 1u;
    check(rx(&f, 1u, "GARBAGE") == 0u &&
              f.state.liveness_misses == 1u,
          "unknown payload gives no liveness credit");
    check(rx(&f, 1u, "MOVE x d3") == 0u &&
              f.state.liveness_misses == 1u,
          "malformed move gives no liveness credit");
    check(rx(&f, 1u, "MACH NXT") == 0u &&
              f.state.liveness_misses == 1u,
          "MACH identity gives no reducer liveness credit");
    check(rx(&f, 1u, "CHAT hello") > 0u &&
              f.state.liveness_misses == 0u,
          "classified payload gives liveness credit");
    ...
    f.state.liveness_misses = 1u;
    rx(&f, 1u, "MOVE 7 d3");
    expect_send(&f, "NACK 7 BUSY", 1u,
                "valid move outside game receives BUSY");
    check(f.state.liveness_misses == 0u,
          "valid outside-game move gives liveness credit");
}

static void test_control_nack_token_boundaries(void)
{
    Fixture f;
    ...
    local_request(&f, SESSION_REQUEST_RESET, 0u, 0);
    tx_result(&f, SESSION_TX_OK);
    check(rx(&f, 1u, "NACK RESETX") == 0u &&
              f.state.pending_control == SESSION_REQUEST_RESET,
          "RESETX does not complete reset");
    rx(&f, 1u, "NACK RESET BUSY");
    check(f.state.pending_control == 0u,
          "boundary-valid reset NACK completes reset");
    ...
    local_request(&f, SESSION_REQUEST_START, 0u, 0);
    tx_result(&f, SESSION_TX_OK);
    check(rx(&f, 1u, "NACK GAME STARTX") == 0u &&
              f.state.pending_control == SESSION_REQUEST_START,
          "STARTX does not complete game start");
}
...
int main(void)
{
    ...
    test_reset_cancel_timeout();
    test_short_tx_is_atomic();
    test_unsupported_draw_and_crossed_reset();
    ...
    test_rx_liveness_requires_classified_payload();
    test_control_nack_token_boundaries();
