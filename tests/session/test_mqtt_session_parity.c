...
static unsigned host_board_redraws;
...
static uint8_t host_end_reason;
...
static uint8_t sntp_capture_enabled;
static uint8_t sntp_clock_ready;
static uint8_t sntp_hour;
static uint8_t sntp_minute;
static uint8_t sntp_second;
static uint16_t sntp_fat_date;
static uint16_t sntp_fat_time;

void spectrum_net_runtime_set_clock(uint8_t hour, uint8_t minute, uint8_t second)
{
    if (!sntp_capture_enabled) {
...
uint8_t spectrum_net_runtime_clock_ready(void) { return sntp_clock_ready; }
                                   ...
                                   const MqttTranscriptObservation *expected)
{
    ...
    uint8_t detail_matches = (uint8_t)(
    ...
    if (expected->type == MQTT_OBSERVE_SESSION) {
        detail_matches = (uint8_t)(
            got->detail ==
            (expected->detail & (uint8_t)~MQTT_OBSERVE_DETAIL_SESSION_REASON));
    }
...
static void print_observation_mismatch(const ActualObservation *actual_values,
                                       uint8_t actual_value_count,
                                       const MqttTranscriptStep *step)
{
    uint8_t i;
    ...
    for (i = 0u; i < actual_value_count; ++i) {
        const ActualObservation *got = &actual_values[i];

        printf("  got  %u/%u/%u/%u/%u/%u/%u/%s\n",
               (unsigned)got->type, (unsigned)got->code,
               (unsigned)got->detail, (unsigned)got->value,
               (unsigned)got->route, (unsigned)got->retained,
               (unsigned)got->link_id, got->payload);
    }
    for (i = 0u; i < step->expected_count; ++i) {
        const MqttTranscriptObservation *want = &step->expected[i];

        printf("  want %u/%u/%u/%u/%u/%u/%u/%s\n",
               (unsigned)want->type, (unsigned)want->code,
               (unsigned)want->detail, (unsigned)want->value,
               (unsigned)want->route, (unsigned)want->retained,
               (unsigned)want->link_id,
               want->payload == 0 ? "" : want->payload);
    }
}
    ...
    const SessionAction *action)
{
    ...
    case SESSION_ACT_SESSION_CHANGED:
        ...
        out->detail = action->data.session.reason;
        break;
    ...
    if (length != 0u) {
        ...
        if (out->type == MQTT_OBSERVE_SEND && length >= 5u &&
            memcmp(payload, "NACK ", 5u) == 0 &&
            (length < 15u || memcmp(payload, "NACK GAME START", 15u) != 0)) {
            out->route = SESSION_ROUTE_ACK;
        }
    }
    return 1u;
}
...
static void host_finish_step(void)
{
    ...
    if (!observations_match(actual, actual_count, step)) {
        ...
        print_observation_mismatch(actual, actual_count, step);
        ++failures;
    }
    host_step_active = 0u;
    actual_count = 0u;
...
static void reset_fixture(const MqttTranscript *transcript)
{
    ...
    host_control_wait_seen = 0u;
    host_unwind_stop = 0u;
    host_end_reason = SESSION_END_REASON_NONE;
...
void spectrum_gui_notify(const char *text, uint8_t is_error)
{
    ...
    if (strcmp(text, NETCHESSZX_UI_ERROR_START_REJECTED) == 0) {
        ...
        uint8_t control = SESSION_REQUEST_RESET;
...
void spectrum_gui_notify_msg(uint16_t packed)
{
    ...
    switch (id) {
    case SPECTRUM_GUI_MSG_START_REJECTED:
        spectrum_gui_notify(NETCHESSZX_UI_ERROR_START_REJECTED, 1u);
        break;
    ...
    if (id == SPECTRUM_GUI_MSG_TAKEBACK_REQUEST) {
...
void spectrum_gui_set_status_error(const char *text)
{
    if (text == msg_host_busy) {
        host_end_reason = SESSION_END_REASON_NONE;
    }
}
...
void spectrum_gui_redraw_board_view(void) { ++host_board_redraws; }
...
uint8_t spectrum_gui_poll_key(void)
{
    ...
    case SESSION_REQUEST_START:
        return game_status_active ? 0u : 32u;
...
int16_t spectrum_net_read_payload(char *payload, uint8_t payload_cap)
{
    ...
    if (host_step_active &&
        host_transcript->steps[host_active_step].event.type ==
            MQTT_TRANSCRIPT_TIMEOUT) {
        if ((control_pending == CONTROL_PENDING_RESET_WAIT) && !host_control_wait_seen) {
        ...
        if (host_timeout_ticks >
            (uint16_t)(NETCHESSZX_SESSION_MQTT_GRACE_POLLS + 1u)) {
...
uint8_t spectrum_gui_board_pieces_visible(void) { return 0u; }
...
uint8_t spectrum_input_parse_move(const char *text, char *move)
{
    ...
    if (length != SPECTRUM_BOARD_MOVE_TEXT_LEN) {
        return 0u;
    }
...
static void sntp_reset_clock(void)
{
    sntp_clock_ready = 0u;
    sntp_hour = 0u;
    sntp_minute = 0u;
...
static void sntp_load_line(const char *line)
{
    uint8_t n = 0u;

    reset_line_buf();
    while (line[n] != '\0' && n + 1u < SPECTRUM_NET_LINE_MAX) {
        line_buf[n] = line[n];
        ++n;
    }
    line_buf[n] = '\0';
...
static void sntp_expect(const char *label, int condition)
{
    if (!condition) {
...
static void check_mqtt_capture_time(void)
{
    sntp_capture_enabled = 1u;

    sntp_reset_clock();
    sntp_load_line("+CIPSNTPTIME:Fri Jun  5 12:34:56 2026");
    sntp_expect("captures SNTP time", mqtt_capture_time_ovl() != 0u);
    sntp_expect("SNTP time fields",
                sntp_clock_ready && sntp_hour == 12u &&
                    sntp_minute == 34u && sntp_second == 56u);
    sntp_expect("SNTP FAT date",
                sntp_fat_date == (uint16_t)(((uint16_t)46u << 9) |
                                            ((uint16_t)6u << 5) | 5u));
    sntp_expect("SNTP FAT time",
                sntp_fat_time == (uint16_t)(((uint16_t)12u << 11) |
                                            ((uint16_t)34u << 5)));

    sntp_reset_clock();
    sntp_load_line("+CIPSNTPTIME:Thu Jan  1 00:00:01 1970");
    sntp_expect("ignores 1970 default time",
                mqtt_capture_time_ovl() == 0u && !sntp_clock_ready);

    sntp_reset_clock();
    sntp_load_line("+CIPSNTPTIME:Fri Jun  5 29:01:02 2026");
    sntp_expect("rejects invalid hour",
                mqtt_capture_time_ovl() == 0u && !sntp_clock_ready);

    sntp_reset_clock();
    sntp_load_line("+CIPSNTPTIME:Fri Jun  5 12:34:56 20K9");
    sntp_expect("rejects non-digit first year suffix",
                mqtt_capture_time_ovl() != 0u && sntp_clock_ready &&
                    sntp_fat_date == 0u && sntp_fat_time == 0u);

    sntp_reset_clock();
    sntp_load_line("+CIPSNTPTIME:Fri Jun  5 12:34:56 201:");
    sntp_expect("rejects non-digit second year suffix",
                mqtt_capture_time_ovl() != 0u && sntp_clock_ready &&
                    sntp_fat_date == 0u && sntp_fat_time == 0u);

    sntp_reset_clock();
    sntp_load_line("OK");
    sntp_expect("ignores non-SNTP lines",
                mqtt_capture_time_ovl() == 0u && !sntp_clock_ready);

    sntp_capture_enabled = 0u;
}

static void check_mqtt_host_ignored_after_game(void)
{
    static char payloads[][8] = {
        "H W 77", "H B 78", "H B 78", "H X 78"
    };
    static const uint8_t retained[] = { 0u, 0u, 1u, 0u };
    uint8_t phase;
    uint8_t i;
    ...
    for (phase = 0u; phase < 2u; ++phase) {
        ...
        mqtt_seat_probed = 1u;
        game_status_active = (uint8_t)(phase == 0u);
        game_over = (uint8_t)(phase != 0u);

        for (i = 0u; i < (uint8_t)(sizeof(retained) / sizeof(retained[0]));
             ++i) {
            uint16_t session_before = netchesszx_mqtt_session_id;
            uint8_t color_before = netchesszx_local_color;
            uint8_t ready_before = netchesszx_session_peer_ready_state;
            uint8_t host_ready_before = netchesszx_host_color_ready;
            uint8_t probed_before = mqtt_seat_probed;

            if (session_presence_handle_event(
                    NETCHESSZX_SESSION_EVENT_MQTT_HOST,
                    payloads[i], retained[i]) !=
                    SESSION_DISPATCH_HANDLED ||
                netchesszx_mqtt_session_id != session_before ||
                netchesszx_local_color != color_before ||
                netchesszx_session_peer_ready_state != ready_before ||
                netchesszx_host_color_ready != host_ready_before ||
                mqtt_seat_probed != probed_before) {
                printf("FAIL: MQTT %s host ignored %u\n",
                       phase == 0u ? "active" : "ended", (unsigned)i);
                ++failures;
            }
        }
    }
...
int main(void)
{
    ...
    check_mqtt_capture_time();
    check_mqtt_host_ignored_after_game();
    check_id_mapping();
    check_mqtt_wire_builders();
    for (i = 0u; i < mqtt_session_transcript_count; ++i) {
        run_shared_transcript(&mqtt_session_transcripts[i]);
    }
    if (host_board_redraws != 0u) {
        printf("FAIL: side announcements repainted fixed-orientation board %u times\n",
               host_board_redraws);
        ++failures;
    }
