...
#define OBS_SESSION_REASON(status, reason) \
    {0, 0u, MQTT_OBSERVE_SESSION, (status), \
     (uint8_t)(MQTT_OBSERVE_DETAIL_SESSION_REASON | (reason)), 0u, 0u, 0u}
#define OBS_SESSION(status) \
    OBS_SESSION_REASON((status), SESSION_END_REASON_NONE)
...
/* Remote-first fixtures need the guest to own white while the remote host
   owns the opening black side. Keep this explicit so actor/turn semantics are
   exercised instead of bypassed by the transcript harness. */
#define GUEST_WHITE_READY_PREFIX \
    {"guest-white link up", EV_LINK_UP(1u), NO_OBSERVATIONS}, \
    { \
        "retained black host selects white guest", \
        EV_RX("H B 77", SESSION_ROUTE_META, SESSION_RX_RETAINED, 1u), \
        {OBS_SIDE(SESSION_COLOR_WHITE, 77u)}, \
        1u \
    }, \
    { \
        "live black host starts white claim", \
        EV_RX("H B 77", SESSION_ROUTE_META, SESSION_RX_LIVE, 1u), \
        { \
    ...
    { \
        "white online completion sends join", \
        EV_TX_OK, \
        { \
            OBS_TIMER_CANCEL(SESSION_TIMER_TX_GUARD), \
            OBS_SEND("J 77", SESSION_ROUTE_META, 0u, 1u), \
    ...
    { \
        "white join completion makes ready", \
        EV_TX_OK, \
        { \
            OBS_TIMER_CANCEL(SESSION_TIMER_TX_GUARD), \
            OBS_SESSION(SESSION_CHANGED_READY), \
...
#define GUEST_WHITE_ACTIVE_PREFIX \
    GUEST_WHITE_READY_PREFIX, \
    { \
        "live black host start sends ack before activation", \
        EV_RX("GAME START", SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u), \
        { \
            OBS_TIMER_CANCEL(SESSION_TIMER_LIVENESS), \
            OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_CONTROL), \
    ...
    { \
        "black start ack completion rearms liveness", \
        EV_TX_OK, \
        { \
            OBS_TIMER_CANCEL(SESSION_TIMER_TX_GUARD), \
            OBS_SESSION(SESSION_CHANGED_STARTED), \
...
#define PLACEMENT_CYCLE(ply_number, ply_text, move_text) \
    { \
        "local placement sends", \
    ...
    { \
        "placement tx completion arms reply", \
        EV_TX_OK, \
        { \
            OBS_TIMER_CANCEL(SESSION_TIMER_TX_GUARD), \
    ...
    { \
        "placement app ack applies move", \
...
#define GUEST_ACTIVE_ONE_PLY_PREFIX_WITH_TIMERS(set_control, cancel_control, \
    ...
    { \
        ...
        EV_LOCAL(SESSION_REQUEST_MOVE, 0u, "d3", SESSION_PHASE_ACTIVE), \
        { \
            OBS_TIMER_CANCEL(SESSION_TIMER_LIVENESS), \
            OBS_SEND("MOVE 1 d3", SESSION_ROUTE_GAME, 0u, 1u), \
            OBS_TIMER_SET(SESSION_TIMER_TX_GUARD, SESSION_TX_GUARD_TICKS) \
        }, \
        3u \
    }, \
    ...
    { \
        ...
        EV_RX("ACK 1 e5", SESSION_ROUTE_ACK, SESSION_RX_LIVE, 1u), \
        { \
            cancel_control(SESSION_TIMER_CONTROL), \
            OBS_GAME(SESSION_DELIVER_LOCAL_MOVE, 0u, 1u, "d3"), \
            OBS_GAME(SESSION_DELIVER_CONTROL_RESULT, \
                     SESSION_CONTROL_ACCEPTED, SESSION_REQUEST_MOVE, "e5"), \
...
#define GUEST_ACTIVE_REMOTE_ONE_PLY_PREFIX \
    ...
    { \
        ...
        EV_RX("MOVE 1 d3 e5", SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u), \
        { \
            OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_LIVENESS), \
            OBS_GAME(SESSION_DELIVER_REMOTE_MOVE, 1u, 1u, "d3"), \
            OBS_INTERNAL_TIMER_SET(SESSION_TIMER_CONTROL, 125u) \
        }, \
        3u \
    }, \
...
static const MqttTranscriptStep seat_tx_fail_before_claim_steps[] = {
    ...
    },
    {
        ...
        EV_TX_FAILED,
        {
            OBS_TIMER_CANCEL(SESSION_TIMER_TX_GUARD),
            OBS_CLOSE(1u),
            OBS_SESSION_REASON(SESSION_CHANGED_ENDED,
...
static const MqttTranscriptStep start_host_retry_limit_steps[] = {
    HOST_READY_PREFIX,
    {
        "zero-ply remote move before game is ignored",
        EV_RX("MOVE 0 d6", SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u),
        NO_STATE_CHANGE
    },
    {
        "malformed remote move before game is ignored",
        EV_RX("MOVE nope", SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u),
        NO_STATE_CHANGE
    },
    {
        "valid remote move before game is busy",
        EV_RX("MOVE 1 d6", SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u),
        {
            OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_LIVENESS),
            OBS_SEND("NACK 1 BUSY", SESSION_ROUTE_ACK, 0u, 1u),
            OBS_TIMER_SET(SESSION_TIMER_TX_GUARD,
                          SESSION_TX_GUARD_TICKS)
        },
        3u
    ...
    {
        "pre-game busy handoff rearms liveness",
        EV_TX_OK,
        {
            OBS_TIMER_CANCEL(SESSION_TIMER_TX_GUARD),
            OBS_TIMER_SET(SESSION_TIMER_LIVENESS, 250u)
...
static const MqttTranscriptStep move_local_ack_steps[] = {
    ...
    },
    {
        ...
        EV_RX("ACK 1 e5", SESSION_ROUTE_ACK, SESSION_RX_RETAINED, 1u),
        NO_OBSERVATIONS
    },
    {
        "live matching ack applies local move and result",
        EV_RX("ACK 1 e5", SESSION_ROUTE_CONTROL, SESSION_RX_LIVE, 1u),
        {
            OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_CONTROL),
            OBS_GAME(SESSION_DELIVER_LOCAL_MOVE, 0u, 1u, "d3"),
            OBS_GAME(SESSION_DELIVER_CONTROL_RESULT,
...
static const MqttTranscriptStep move_remote_accept_steps[] = {
    HOST_ACTIVE_PREFIX,
    {
        ...
        EV_RX("MOVE 1 d3 e5", SESSION_ROUTE_GAME, SESSION_RX_RETAINED, 1u),
        NO_STATE_CHANGE
    },
    {
        "cross-topic meta move is inert",
        EV_RX("MOVE 1 d3 e5", SESSION_ROUTE_META, SESSION_RX_LIVE, 1u),
        NO_STATE_CHANGE
    },
    {
...
static const MqttTranscriptStep move_remote_reject_steps[] = {
    HOST_ACTIVE_PREFIX,
    {
        ...
        EV_RX("MOVE 1 a1", SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u),
        {
            OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_LIVENESS),
            OBS_GAME(SESSION_DELIVER_REMOTE_MOVE, 1u, 1u, "a1"),
            OBS_INTERNAL_TIMER_SET(SESSION_TIMER_CONTROL, 125u)
        },
        3u
    },
    ...
    {
        "corrected placement at rejected ply is delivered",
        EV_RX("MOVE 1 d3", SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u),
        {
            OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_LIVENESS),
            OBS_GAME(SESSION_DELIVER_REMOTE_MOVE, 2u, 1u, "d3"),
            OBS_INTERNAL_TIMER_SET(SESSION_TIMER_CONTROL, 125u)
        },
        3u
    },
    {
        "corrected placement acceptance is acknowledged",
        EV_GAME_RESULT(SESSION_GAME_ACCEPTED, 1u, 0),
        {
            OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_LIVENESS),
            OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_CONTROL),
...
static const MqttTranscriptStep move_sync_and_tx_fail_steps[] = {
    ...
    {
        "inherited four-byte local move is ignored",
        EV_LOCAL(SESSION_REQUEST_MOVE, 0u, "e2e4", SESSION_PHASE_ACTIVE),
        NO_STATE_CHANGE
    },
    {
        "inherited five-byte local move is ignored",
        EV_LOCAL(SESSION_REQUEST_MOVE, 0u, "e7e8q", SESSION_PHASE_ACTIVE),
        NO_STATE_CHANGE
    },
    {
        "out-of-sequence remote move is nacked",
        EV_RX("MOVE 2 c4", SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u),
        {
            OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_LIVENESS),
            OBS_SEND_NACK_COMPAT("NACK 2", SESSION_ROUTE_ACK, 0u, 1u),
            OBS_TIMER_SET(SESSION_TIMER_TX_GUARD, SESSION_TX_GUARD_TICKS)
        },
        3u
    },
...
static const MqttTranscriptStep move_placement_steps[] = {
    GUEST_ACTIVE_PREFIX,
    PLACEMENT_CYCLE(1u, "1", "d3"),
    PLACEMENT_CYCLE(2u, "2", "c3"),
    PLACEMENT_CYCLE(3u, "3", "c4"),
    PLACEMENT_CYCLE(4u, "4", "c5")
};

#define RESTORE_CHUNK0 "AAAAAAAAAAAAAAAAAAIQAAABIAAAAA"
#define RESTORE_CHUNK0_OVER "ERERERERERERERERERERERERERERER"
#define RESTORE_CHUNK1_READY "AAAAAAAAAAAABAAAAAAAEAAAAAOwLM"
#define RESTORE_CHUNK1_ACTIVE "AAAAAAAAAAAABBAAIAAQEAAAAAOwJS"
#define RESTORE_CHUNK1_OVER "ERERERERERERFEAAQAAgEAAAAAOwJ1"
...
static const MqttTranscriptStep restore_local_active_steps[] = {
    ...
    },
    {
        ...
        {
            OBS_TIMER_CANCEL(SESSION_TIMER_LIVENESS),
            OBS_SEND("MOVE 3 d3", SESSION_ROUTE_GAME, 0u, 1u),
            OBS_TIMER_SET(SESSION_TIMER_TX_GUARD,
                          SESSION_TX_GUARD_TICKS)
        },
        3u
    ...
    },
    {
        ...
        {
            OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_CONTROL),
            OBS_GAME(SESSION_DELIVER_LOCAL_MOVE, 0u, 3u, "d3"),
            OBS_GAME(SESSION_DELIVER_CONTROL_RESULT,
                     SESSION_CONTROL_ACCEPTED,
                     SESSION_REQUEST_MOVE, 0),
            OBS_TIMER_SET(SESSION_TIMER_LIVENESS, 250u)
...
static const MqttTranscriptStep restore_remote_fresh_steps[] = {
    ...
    {
        "local chat handoff preserves restore cache",
        EV_TX_OK,
        {
            OBS_TIMER_CANCEL(SESSION_TIMER_TX_GUARD),
...
#define RESTORE_REPLACEMENT_WAIT_RA_STEPS(host, color, sid, online, join) \
...
static const MqttTranscriptStep restore_replace_applied_cache_steps[] = {
    GUEST_READY_PREFIX,
    {
        "remote restore request opens prompt",
        EV_RX("RQ", SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u),
        {OBS_DECISION(SESSION_REQUEST_RESTORE, 0u)}, 1u
    },
    {
        "accepted restore request sends permission",
        EV_DECISION(SESSION_CONTROL_ACCEPTED),
        {OBS_SEND("RY", SESSION_ROUTE_GAME, 0u, 1u),
         OBS_TIMER_SET(SESSION_TIMER_TX_GUARD, SESSION_TX_GUARD_TICKS)}, 2u
    },
    {
        "restore permission handoff starts receive", EV_TX_OK,
        {OBS_TIMER_CANCEL(SESSION_TIMER_TX_GUARD),
         OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_LIVENESS),
         OBS_INTERNAL_TIMER_SET(SESSION_TIMER_CONTROL, 125u)}, 3u
    },
    {
        "first restore half is cached",
        EV_RX(RESTORE_RS00, SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u),
        {OBS_INTERNAL_TIMER_SET(SESSION_TIMER_CONTROL, 125u)}, 1u
    },
    {
        "second restore half is delivered",
        EV_RX(RESTORE_RS01_READY, SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u),
        {OBS_GAME(SESSION_DELIVER_RESTORE, 2u, 0u, RESTORE_READY),
         OBS_INTERNAL_TIMER_SET(SESSION_TIMER_CONTROL, 125u)}, 2u
    },
    {
        ...
        {OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_CONTROL),
         OBS_SEND("RA", SESSION_ROUTE_GAME, 0u, 1u),
         OBS_TIMER_SET(SESSION_TIMER_TX_GUARD, SESSION_TX_GUARD_TICKS)}, 3u
    },
    {
        "restore acknowledgement handoff caches apply", EV_TX_OK,
        {OBS_TIMER_CANCEL(SESSION_TIMER_TX_GUARD),
         OBS_INTERNAL_TIMER_SET(SESSION_TIMER_LIVENESS, 250u)}, 2u
    },
    {
        ...
        {OBS_SIDE(SESSION_COLOR_BLACK, 78u),
         OBS_TIMER_CANCEL(SESSION_TIMER_LIVENESS),
         OBS_SEND("O B 78", SESSION_ROUTE_PRESENCE, 1u, 1u),
         OBS_TIMER_SET(SESSION_TIMER_TX_GUARD, SESSION_TX_GUARD_TICKS)}, 4u
    },
    GUEST_REPLACEMENT_HANDSHAKE("J 78"),
    {
        "old cached chunk is refused instead of reacked",
        EV_RX(RESTORE_RS00, SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u),
        {OBS_SEND("RN", SESSION_ROUTE_GAME, 0u, 1u),
         OBS_TIMER_SET(SESSION_TIMER_TX_GUARD, SESSION_TX_GUARD_TICKS)}, 2u
    }
...
static const MqttTranscriptStep control_local_reset_steps[] = {
    ...
    {
        "reset nack prefix collision leaves request pending",
        EV_RX("NACK RESETX", SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u),
        NO_STATE_CHANGE
    },
    {
        "reset nack is typed",
...
static const MqttTranscriptStep control_unsupported_draw_steps[] = {
    GUEST_ACTIVE_PREFIX,
    {
        "legacy draw is inert without liveness credit",
        EV_RX("DRAW", SESSION_ROUTE_CONTROL, SESSION_RX_LIVE, 1u),
        NO_STATE_CHANGE
    },
    {
        "legacy draw ack is inert without liveness credit",
        EV_RX("ACK DRAW", SESSION_ROUTE_ACK, SESSION_RX_LIVE, 1u),
        NO_STATE_CHANGE
    },
    {
        "legacy draw nack is inert without liveness credit",
        EV_RX("NACK DRAW", SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u),
        NO_STATE_CHANGE
    },
    {
        "legacy draw nack advisory is inert without liveness credit",
        EV_RX("NACK DRAW BUSY", SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u),
        NO_STATE_CHANGE
    },
    {
        "legacy draw cancel is inert without liveness credit",
        EV_RX("CANCEL DRAW", SESSION_ROUTE_CONTROL, SESSION_RX_LIVE, 1u),
        NO_STATE_CHANGE
    }
};
...
static const MqttTranscriptStep control_remote_resign_steps[] = {
    ...
    {
        "valid remote move after resign is busy",
        EV_RX("MOVE 1 d6", SESSION_ROUTE_GAME, SESSION_RX_LIVE, 1u),
        {
            OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_LIVENESS),
            OBS_SEND("NACK 1 BUSY", SESSION_ROUTE_ACK, 0u, 1u),
    ...
    {
        "post-game busy handoff rearms liveness",
        EV_TX_OK,
        {
            OBS_TIMER_CANCEL(SESSION_TIMER_TX_GUARD),
            OBS_TIMER_SET(SESSION_TIMER_LIVENESS, 250u)
...
static const MqttTranscriptStep liveness_peer_timeout_steps[] = {
    ...
    {
        "live host reuses surviving broker link",
        EV_RX("H W 77", SESSION_ROUTE_META, SESSION_RX_LIVE, 1u),
        {
            OBS_SEND("J 77", SESSION_ROUTE_META, 0u, 1u),
            OBS_TIMER_SET(SESSION_TIMER_TX_GUARD, SESSION_TX_GUARD_TICKS)
    ...
    {
        "recovery join handoff restores ready",
        EV_TX_OK,
        {
            OBS_TIMER_CANCEL(SESSION_TIMER_TX_GUARD),
            OBS_SESSION(SESSION_CHANGED_READY),
...
static const MqttTranscriptStep liveness_host_peer_timeout_steps[] = {
    ...
    {
        "replacement join receives live host without link-up",
        EV_RX("J 77", SESSION_ROUTE_META, SESSION_RX_LIVE, 1u),
        {
            OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_CONTROL),
            OBS_SEND("H W 77", SESSION_ROUTE_META, 0u, 1u),
...
static const MqttTranscriptStep link_loss_pending_tx_fresh_steps[] = {
    GUEST_ACTIVE_ONE_PLY_PRIVATE_PREFIX,
    {
        ...
        EV_LOCAL(SESSION_REQUEST_MOVE, 0u, "c3", SESSION_PHASE_ACTIVE),
        {
            OBS_TIMER_CANCEL(SESSION_TIMER_LIVENESS),
            OBS_SEND("MOVE 2 c3", SESSION_ROUTE_GAME, 0u, 1u),
            OBS_TIMER_SET(SESSION_TIMER_TX_GUARD, SESSION_TX_GUARD_TICKS)
        },
        3u
    },
    ...
    },
    {
        ...
        {
            OBS_TIMER_CANCEL(SESSION_TIMER_LIVENESS),
            OBS_SEND("MOVE 1 d3", SESSION_ROUTE_GAME, 0u, 2u),
            OBS_TIMER_SET(SESSION_TIMER_TX_GUARD, SESSION_TX_GUARD_TICKS)
        },
        3u
    }
...
static const MqttTranscriptStep link_loss_duplicate_fresh_steps[] = {
    ...
    },
    {
        ...
        EV_RX("MOVE 1 d3", SESSION_ROUTE_GAME, SESSION_RX_LIVE, 2u),
        {
            OBS_INTERNAL_TIMER_CANCEL(SESSION_TIMER_LIVENESS),
            OBS_GAME(SESSION_DELIVER_REMOTE_MOVE, 1u, 1u, "d3"),
            OBS_INTERNAL_TIMER_SET(SESSION_TIMER_CONTROL, 125u)
...
static const MqttTranscriptStep host_ignored_active_steps[] = {
    GUEST_ACTIVE_PREFIX,
    {
        "active exact live host is inert",
        EV_RX("H W 77", SESSION_ROUTE_META, SESSION_RX_LIVE, 1u),
        NO_STATE_CHANGE
    },
    {
        "active replacement live host is inert",
        EV_RX("H B 78", SESSION_ROUTE_META, SESSION_RX_LIVE, 1u),
        NO_STATE_CHANGE
    },
    {
        "active retained host is inert",
        EV_RX("H B 78", SESSION_ROUTE_META, SESSION_RX_RETAINED, 1u),
        NO_STATE_CHANGE
    },
    {
        "active malformed host is inert",
        EV_RX("H X 78", SESSION_ROUTE_META, SESSION_RX_LIVE, 1u),
        NO_STATE_CHANGE
    }
};
...
const MqttTranscript mqtt_session_transcripts[] = {
    {
        "mqtt-host-ignored-active",
        host_ignored_active_steps,
        0u,
        SESSION_ROLE_GUEST,
        SESSION_COLOR_UNKNOWN,
        (uint8_t)(sizeof(host_ignored_active_steps) /
                  sizeof(host_ignored_active_steps[0]))
    },
    {
    ...
    {
        "mqtt-move-placement-sequence",
        move_placement_steps,
        0u,
        SESSION_ROLE_GUEST,
        SESSION_COLOR_UNKNOWN,
        (uint8_t)(sizeof(move_placement_steps) /
                  sizeof(move_placement_steps[0]))
    },
    {
    ...
    {
        "mqtt-control-unsupported-draw",
        control_unsupported_draw_steps,
        0u,
        SESSION_ROLE_GUEST,
        SESSION_COLOR_UNKNOWN,
        (uint8_t)(sizeof(control_unsupported_draw_steps) /
                  sizeof(control_unsupported_draw_steps[0]))
    },
    {
