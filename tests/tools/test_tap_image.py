...
def main() -> int:
    ...
    cart_symbols = dict(SYMBOLS, __BSS_END_tail=0x5B20,
                        __bss_user_tail=0x5B20, __bss_compiler_tail=ORG + 5)
    low_code = b"\xc9" * 100
    packed = low_code + bytes(512 - len(low_code)) + code
    image = validate_image(cart_symbols, code, code_tap(packed, ORG - 512),
                           ORG, ORG - 512, low_code)
    assert image.bss_bytes == 2 and image.payload == packed
    for bad_low in (b"\x00" * 100, bytes(513)):
        try:
            validate_image(cart_symbols, code, code_tap(packed, ORG - 512),
                           ORG, ORG - 512, bad_low)
        except TapImageError:
            pass
        else:
            raise AssertionError("corrupt/oversize cartridge backend accepted")
