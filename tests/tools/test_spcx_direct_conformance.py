...
class DirectConformanceTests(unittest.TestCase):
    ...
    def test_spcx_machine_code_accepted(self) -> None:
        clock = FakeClock()
        host = DirectPeer("host", machine="SPCX", clock=clock)
        guest = DirectPeer("guest", machine="ZX", clock=clock)
        exchange(host, guest)
        self.assertTrue(host.ready)
        self.assertTrue(guest.ready)
        self.assertEqual(host.peer_machine, "ZX")
        self.assertEqual(guest.peer_machine, "SPCX")
